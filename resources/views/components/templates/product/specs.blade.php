...
<x-templates.product.shell active="Specs">
    ...
    <div class="mt-8 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_20rem]">
            ...
            @foreach ($groups as $group)
                <section class="overflow-hidden rounded-2xl border border-white/8 bg-ink-900">
                    <div class="flex items-baseline justify-between gap-4 border-b border-white/5 px-5 py-3">
                        ...
                        <span class="font-mono text-[10px] text-zinc-700">{{ sprintf('%02d', count($group['rows'])) }}</span>
                    </div>

                    <div class="flex flex-col divide-y divide-white/5">
                        @foreach ($group['rows'] as $row)
                            <x-templates.product.spec-row :label="$row['label']" :value="$row['value']" :note="$row['note'] ?? null" />
                        @endforeach
                    </div>
                </section>
            @endforeach
        ...
        <div class="flex flex-col gap-4 lg:sticky lg:top-32">
            ...
            <div class="flex flex-col divide-y divide-white/5 overflow-hidden rounded-2xl border border-white/8 bg-ink-900">
                @foreach ([['Spec sheet', 'PDF · 240 kB'], ['Exploded view and parts list', 'PDF · 1.1 MB'], ['Step chart, printable', 'PDF · 90 kB'], ['CAD, front and side', 'DXF · 380 kB']] as $file)
                    <a href="#" class="flex items-center gap-3 px-4 py-3 transition-colors duration-150 hover:bg-white/4">
                    ...
                    @foreach ($compare as $row)
                        <tr>
                            ...
                            @foreach ($row['values'] as $index => $value)
                                <td @class(['border-b px-4 py-3 align-middle', 'border-jade-500/20 bg-jade-500/6' => $index === 0, 'border-white/5' => $index !== 0])>
                                    @if ($value === true)
                                        <svg class="size-3.5 text-jade-400" viewBox="0 0 12 12" fill="none" role="img" aria-label="Yes">
                                            <path d="M2 6.5 4.5 9 10 3" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                    @elseif ($value === false)
                                        <span class="block h-3.5 w-3 border-b border-white/15" role="img" aria-label="No"></span>
                                    @else
                                        <span class="font-mono text-[12px] text-zinc-400">{{ $value }}</span>
                                    @endif
                                </td>
                            @endforeach
                        </tr>
