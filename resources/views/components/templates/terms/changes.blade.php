...
<x-templates.terms.shell active="What changed" :rail="false">
    <x-slot:toolbar>
        <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
            <span class="font-mono text-[10px] text-zinc-600">Seven versions since April 2019</span>
            <span class="font-mono text-[10px] text-zinc-700">promised notice 30 days · this one got 45</span>
            <a href="{{ route('templates.screen', ['terms', 'document']) }}" target="_top"
                class="ml-auto font-mono text-[11px] text-jade-400 transition-colors duration-150 hover:text-jade-300">read 4.1 in full →</a>
        </div>
    </x-slot:toolbar>

    <div data-changes class="mx-auto max-w-5xl">

        <h1 class="text-lg font-semibold tracking-tight text-cream">Every version, and why it moved</h1>
        ...
        <x-templates.terms.notice
            ...
            <x-slot:actions>
                <a href="{{ route('templates.screen', ['terms', 'record']) }}" target="_top"
                    class="rounded-lg border border-white/12 px-3 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream">See which version you are on</a>
            </x-slot>
        </x-templates.terms.notice>

        <div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,18rem)_1fr]">
            ...
            <div>
                @foreach ($versions as $version)
                    <div data-version-panel="{{ $version['version'] }}" @class(['hidden' => ! $loop->first])>
                        <div class="flex flex-wrap items-baseline gap-x-3 gap-y-1.5">
                            <h2 class="text-[15px] font-medium tracking-tight text-cream">What {{ $version['version'] }} did</h2>
                            <span class="font-mono text-[11px] text-zinc-700">{{ $version['date'] }}</span>
                        </div>

                        <p class="mt-2 max-w-2xl text-[13px]/6 text-zinc-500">{{ $version['lead'] }}</p>

                        <p class="mt-3 flex flex-wrap items-baseline gap-x-2 gap-y-1">
                            <span class="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Who asked for it</span>
                            <span class="text-[12px]/5 text-zinc-400">{{ $version['asked'] }}</span>
                        </p>

                        @if ($version['note'])
                            <p class="mt-5 rounded-xl border border-white/8 bg-ink-900 p-4 text-[12px]/5 text-zinc-500">{{ $version['note'] }}</p>
                        @endif

                        @if ($version['diffs'] !== [])
                            <div class="mt-5 flex flex-col gap-3">
                                @foreach ($version['diffs'] as $diff)
                ...
                <section class="mt-8 rounded-xl border border-white/8 bg-ink-900 p-4">
                    <h2 class="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">How a change happens here</h2>
                    <div class="mt-3.5 grid grid-cols-1 gap-x-6 gap-y-3.5 sm:grid-cols-2">
                        @foreach ($how as $entry)
    ...
    <script>
        (() => {
            const root = document.querySelector('[data-changes]');

            if (!root) {
                return;
            }

            const buttons = [...root.querySelectorAll('[data-revision]')];
            const panels = [...root.querySelectorAll('[data-version-panel]')];

            buttons.forEach((button) => {
                button.addEventListener('click', () => {
                    const picked = button.dataset.revision;

                    buttons.forEach((other) => other.toggleAttribute('data-active', other === button));
                    panels.forEach((panel) => panel.classList.toggle('hidden', panel.dataset.versionPanel !== picked));
                });
            });
        })();
    </script>
