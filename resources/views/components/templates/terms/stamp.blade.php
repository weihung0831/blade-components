@props([
    'version',
    'state' => 'accepted',
    'when' => null,
    'rows' => [],
    'hash' => null,
    'note' => null,
])

@php
    $pill = [
        'accepted' => ['label' => 'accepted', 'class' => 'border-jade-500/40 bg-jade-500/10 text-jade-300'],
        'waiting' => ['label' => 'not yet accepted', 'class' => 'border-amber-400/30 bg-amber-400/8 text-amber-300/90'],
        'superseded' => ['label' => 'superseded', 'class' => 'border-white/10 text-zinc-600'],
    ][$state] ?? ['label' => $state, 'class' => 'border-white/10 text-zinc-600'];
@endphp

<div {{ $attributes->class('rounded-xl border border-white/8 bg-ink-900 p-4') }}>

    <div class="flex flex-wrap items-baseline gap-x-3 gap-y-1.5">
        <span class="font-mono text-xl text-cream">{{ $version }}</span>
        <span class="rounded border px-1.5 py-0.5 font-mono text-[10px] {{ $pill['class'] }}">{{ $pill['label'] }}</span>
        @if ($when)
            <span class="ml-auto font-mono text-[11px] text-zinc-600">{{ $when }}</span>
        @endif
    </div>

    @if ($rows !== [])
        <dl class="mt-3.5 border-t border-dashed border-white/10 pt-3.5">
            @foreach ($rows as $row)
                <div class="flex gap-4 py-1">
                    <dt class="w-28 shrink-0 font-mono text-[10px] text-zinc-700 uppercase">{{ $row['label'] }}</dt>
                    <dd @class([
                        'min-w-0 flex-1 text-[12px]/5',
                        'font-mono text-zinc-300' => $row['mono'] ?? false,
                        'text-zinc-400' => ! ($row['mono'] ?? false),
                    ])>{{ $row['value'] }}</dd>
                </div>
            @endforeach
        </dl>
    @endif

    @if ($hash)
        <p class="mt-3 truncate border-t border-dashed border-white/10 pt-3 font-mono text-[10px] text-zinc-700">
            sha256 {{ $hash }}
        </p>
    @endif

    @if ($note)
        <p class="mt-3 text-[11px]/5 text-zinc-600">{{ $note }}</p>
    @endif

    @if (isset($actions))
        <div class="mt-3.5 flex flex-wrap gap-2">{{ $actions }}</div>
    @endif
</div>
