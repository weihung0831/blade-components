...
@php
    ...
    $order = array_column($plan, 'key');
    $at = array_search($step, $order, true);
    $at = $at === false ? count($order) : $at;

    $steps = [];

    foreach ($plan as $index => $entry) {
        $steps[] = $entry + ['state' => match (true) {
            in_array($entry['key'], $skipped, true) => 'skipped',
            $index < $at => 'done',
            $index === $at => 'current',
            default => 'todo',
        }];
...
<div {{ $attributes->class('flex h-full w-full flex-col overflow-hidden bg-ink-950') }}>
    ...
    <header class="shrink-0 border-b border-white/5 bg-ink-950">
        <div class="flex h-14 items-center gap-5 px-4 sm:px-5">
            <a href="{{ route('templates.screen', ['onboarding', 'setup']) }}" target="_top" class="flex shrink-0 items-center gap-2.5">
            ...
            <nav class="hidden items-center gap-1 md:flex">
                @foreach ($links as $link)
                    <a href="{{ route('templates.screen', ['onboarding', $link['screen']]) }}" target="_top"
            ...
            <div class="ml-auto flex shrink-0 items-center gap-3">
                ...
                <span data-shell-position class="font-mono text-[11px] text-zinc-500">step {{ $position }} of {{ count($plan) }}</span>
        ...
        <div class="h-0.5 w-full bg-white/5">
            <div data-shell-bar class="h-full bg-jade-500 transition-[width] duration-300" style="width: {{ round($position / count($plan) * 100) }}%"></div>
        </div>
    ...
    <div class="relative flex min-h-0 flex-1">
        @if ($rail)
            <aside class="hidden w-64 shrink-0 flex-col justify-between overflow-y-auto border-r border-white/5 py-4 lg:flex">
                <div>
                    <p class="px-4 font-mono text-[10px] tracking-wider text-zinc-700 uppercase">The whole thing, up front</p>

                    <x-templates.onboarding.stepper class="mt-3 px-4" :steps="$steps" :interactive="$interactive" />
                ...
                <div class="mx-2 mt-6 rounded-xl border border-white/8 bg-ink-900 p-3">
                    ...
                    <a href="{{ route('templates.screen', ['onboarding', 'dropout']) }}" target="_top"
                        class="mt-2.5 block rounded-lg border border-white/10 py-1.5 text-center text-[12px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream">How far people actually get</a>
                </div>
            </aside>
        @endif
