@php
    ...
    $rowCount = array_sum(array_map(fn (array $group): int => count($group['rows']), $groups));
@endphp

<x-templates.pricing.shell active="Compare" title="Every row, so you can find the one you came for."
            ...
            <thead>
                <tr>
                    ...
                    @foreach ($tiers as $tier)
                        <th scope="col" @class([
                            'z-10 border-b bg-ink-950 px-4 py-4 align-bottom lg:sticky lg:top-14',
                            "relative isolate border-jade-500/40 before:absolute before:inset-0 before:-z-10 before:bg-jade-500/6 before:content-['']" => $tier['featured'] ?? false,
                            'border-white/8' => ! ($tier['featured'] ?? false),
                        ])>
                            <div class="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                                <span @class(['text-sm font-semibold', ($tier['featured'] ?? false) ? 'text-jade-300' : 'text-cream'])>{{ $tier['name'] }}</span>
                        ...
                        <tr class="group/row">
                            ...
                            @foreach ($row['values'] as $index => $value)
                                <td @class([
                                    ...
                                    'border-jade-500/20 bg-jade-500/6' => $tiers[$index]['featured'] ?? false,
                                    'border-white/5' => ! ($tiers[$index]['featured'] ?? false),
                                ])>
                                    @if ($value === true)
