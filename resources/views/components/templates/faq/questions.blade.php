@php
    $topics = [
        ['name' => 'Noise and grind', 'count' => 4, 'lead' => 'It howls above half the dial after three weeks.', 'health' => 91],
        ['name' => 'Setting it up', 'count' => 3, 'lead' => 'How much coffee do I waste seasoning the burrs?', 'health' => 88],
        ['name' => 'Warranty', 'count' => 3, 'lead' => 'I opened it myself. Have I voided anything?', 'health' => 62, 'note' => '2 need a rewrite'],
        ['name' => 'Orders and delivery', 'count' => 3, 'lead' => 'Nine days and the tracking has not moved.', 'health' => 84],
        ['name' => 'Before you buy', 'count' => 2, 'lead' => 'Jade or graphite — does the finish change anything?', 'health' => 96],
        ['name' => 'Dealers', 'count' => 1, 'lead' => 'What do dealer terms look like for two shops?', 'health' => 90],
    ];
    ...
    $searched = ['noise', 'batch 40', 'warranty', 'burr gap', 'shipping', 'static', 'refund'];
@endphp

<x-templates.faq.shell active="Answers" :padded="false">
    <div data-faq-screen class="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <div class="shrink-0 border-b border-white/5 px-5 py-5">
            <div class="flex flex-wrap items-end justify-between gap-4">
                ...
                <button type="button" data-expand
                    class="shrink-0 rounded-lg border border-white/10 px-3 py-1.5 font-mono text-[11px] text-zinc-500 transition-colors duration-150 hover:border-jade-500/50 hover:text-cream">open every one</button>
            </div>
            ...
            <label class="relative mt-4 flex max-w-xl items-center">
                ...
                <input type="search" data-search spellcheck="false"
            ...
            <div class="mt-2.5 flex flex-wrap items-center gap-1.5">
                ...
                @foreach ($searched as $term)
                    <button type="button" data-term="{{ $term }}"
                        class="rounded-md border border-white/10 px-1.5 py-0.5 font-mono text-[10px] text-zinc-500 transition-colors duration-150 hover:border-jade-500/50 hover:text-cream">{{ $term }}</button>
                @endforeach
            </div>
        </div>

        <div data-ui-scroll-region class="min-h-0 flex-1 overflow-y-auto">
            ...
            <div class="flex items-center gap-3 border-y border-white/5 bg-ink-900/40 px-5 py-2">
                <p class="font-mono text-[10px] text-zinc-600">
                    showing <span data-count class="text-zinc-400">{{ count($questions) }}</span> of {{ count($questions) }}
                    <span data-scope class="text-zinc-700"></span>
                </p>
                <button type="button" data-clear class="ml-auto hidden font-mono text-[10px] text-zinc-600 transition-colors duration-150 hover:text-cream">clear</button>
            </div>
            ...
            <div data-list>
                @foreach ($questions as $entry)
                    <x-templates.faq.question
                        ...
                        @foreach ($entry['a'] as $paragraph)
                            <p>{{ $paragraph }}</p>
                        @endforeach

                        @if (isset($entry['note']))
                            <x-templates.faq.callout :tone="$entry['note']['tone']" :label="$entry['note']['label']" class="mt-4">
                                <p>{{ $entry['note']['text'] }}</p>
                        ...
                        <x-slot:footer>
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                <span class="font-mono text-[10px] text-zinc-700">opened {{ number_format($entry['opens']) }} times</span>

                                <a href="{{ route('templates.screen', ['faq', 'ask']) }}" target="_top"
                                    class="ml-auto font-mono text-[10px] text-zinc-600 transition-colors duration-150 hover:text-cream">this did not answer it →</a>
                            </div>
                        </x-slot>
                    </x-templates.faq.question>
                @endforeach

                <div data-empty class="hidden px-5 py-16 text-center">
                    ...
                    <a href="{{ route('templates.screen', ['faq', 'ask']) }}" target="_top"
                        class="mt-4 inline-block rounded-lg bg-jade-500 px-3.5 py-2 text-[13px] font-medium text-ink-950 transition-colors duration-150 hover:bg-jade-400">Ask the desk instead</a>
                </div>
            </div>
            ...
            <div class="px-5 py-8">
                <div class="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-white/8 bg-ink-900 px-5 py-4">
                    ...
                    <a href="{{ route('templates.screen', ['faq', 'ask']) }}" target="_top"
                        class="shrink-0 rounded-lg border border-white/10 px-3.5 py-2 text-[13px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream">Write to the desk</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        (() => {
            const screen = document.querySelector('[data-faq-screen]');
            ...
            const rows = [...screen.querySelectorAll('[data-question]')];
            const cards = [...screen.querySelectorAll('[data-topic-card]')];
            const counter = screen.querySelector('[data-count]');
            const scope = screen.querySelector('[data-scope]');
            const empty = screen.querySelector('[data-empty]');
            const clear = screen.querySelector('[data-clear]');
            const expand = screen.querySelector('[data-expand]');
            ...
            const run = () => {
                ...
                const shown = rows.filter((row) => {
                    const match = (topic === null || row.dataset.topic === topic)
                ...
                clear.classList.toggle('hidden', topic === null && term === '');

                scope.textContent = topic === null ? '' : `· ${topic}`;

                cards.forEach((card) => card.toggleAttribute('data-active', card.dataset.topicCard === topic));

                if (shown.length === 1 && term !== '') {
                    shown[0].open = true;
                }
            };

            cards.forEach((card) => card.addEventListener('click', () => {
                topic = topic === card.dataset.topicCard ? null : card.dataset.topicCard;
                run();
            }));

            screen.querySelectorAll('[data-term]').forEach((button) => button.addEventListener('click', () => {
                input.value = input.value.trim().toLowerCase() === button.dataset.term ? '' : button.dataset.term;
                run();
            }));

            expand.addEventListener('click', () => {
                const visible = rows.filter((row) => !row.classList.contains('hidden'));
                const opening = visible.some((row) => !row.open);

                visible.forEach((row) => { row.open = opening; });

                expand.textContent = opening ? 'close every one' : 'open every one';
            });
