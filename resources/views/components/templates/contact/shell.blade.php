...
@php
    ...
    $routes = [
        ['label' => 'Warranty and repairs', 'reply' => '47 min', 'screen' => 'write'],
        ['label' => 'Orders and delivery', 'reply' => '2 h', 'screen' => 'write'],
        ['label' => 'Dealers and wholesale', 'reply' => '1 day', 'screen' => 'write'],
        ['label' => 'Press and everything else', 'reply' => '3 days', 'screen' => 'write'],
    ];
...
<div {{ $attributes->class('flex h-full w-full flex-col overflow-hidden bg-ink-950') }}>
    ...
    <header class="shrink-0 border-b border-white/5 bg-ink-950">
        <div class="flex h-14 items-center gap-5 px-4 sm:px-5">
            <a href="{{ route('templates.screen', ['contact', 'desk']) }}" target="_top" class="flex shrink-0 items-center gap-2.5">
            ...
            <nav class="hidden items-center gap-1 md:flex">
                @foreach ($links as $link)
                    <a href="{{ route('templates.screen', ['contact', $link['screen']]) }}" target="_top"
            ...
            <aside class="hidden w-56 shrink-0 flex-col justify-between overflow-y-auto border-r border-white/5 py-4 lg:flex">
                <div>
                    ...
                    <nav class="mt-2 px-2">
                        @foreach ($routes as $entry)
                            <a href="{{ route('templates.screen', ['contact', $entry['screen']]) }}" target="_top"
                    ...
                    <div class="mt-2 px-4">
                        ...
                        <a href="{{ route('templates.screen', ['contact', 'visit']) }}" target="_top"
                            class="mt-1.5 inline-block font-mono text-[11px] text-jade-400 transition-colors duration-150 hover:text-jade-300">how to get in →</a>
                    </div>
                </div>
                ...
                <div class="mx-2 mt-6 rounded-xl border border-white/8 bg-ink-900 p-3">
                    ...
                    <a href="{{ route('templates.screen', ['faq', 'questions']) }}" target="_top"
                        class="mt-2.5 block rounded-lg border border-white/10 py-1.5 text-center text-[12px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream">Look there first</a>
                </div>
            </aside>
        @endif
