@php
    ...
    $total = array_sum(array_map('count', $years));
@endphp

<x-templates.blog.shell active="Archive">
    <div data-blog-archive data-topic="all">
        ...
        <div class="mt-8 flex flex-col gap-4">
            <div class="relative">
                ...
                <input type="search" data-archive-search placeholder="Search titles and topics" aria-label="Search the archive"
                ...
                <span class="absolute top-1/2 right-4 -translate-y-1/2 font-mono text-[10px] text-zinc-600">
                    <span data-archive-count>{{ $total }}</span> shown
                </span>
            </div>

            <div class="flex flex-wrap items-center gap-2">
                <x-templates.blog.topic-pill value="all" label="Everything" :count="$total" checked />
                @foreach ($topics as $topic)
                    @php
                        $count = count(array_filter(array_merge(...array_values($years)), fn (array $note): bool => $note['topic'] === $topic));
                    @endphp
                    <x-templates.blog.topic-pill :value="$topic" :label="$topic" :count="$count" />
                @endforeach
            </div>
        </div>

        <div class="mt-10 flex flex-col gap-10">
            @foreach ($years as $year => $notes)
                <section data-year-group="{{ $year }}">
                    <div class="flex items-baseline gap-4">
                        <h2 class="font-mono text-sm tracking-wider text-cream">{{ $year }}</h2>
                        <span class="h-px flex-1 bg-white/8"></span>
                        <span data-year-count="{{ $year }}" class="font-mono text-[10px] text-zinc-600">{{ count($notes) }} {{ Illuminate\Support\Str::plural('note', count($notes)) }}</span>
                    </div>

                    <ol class="mt-2 flex flex-col divide-y divide-white/5">
                        @foreach ($notes as $note)
                            <li data-archive-row data-topic="{{ $note['topic'] }}"
                                data-search="{{ Illuminate\Support\Str::lower($note['title'].' '.$note['topic']) }}">
            ...
            <div data-archive-empty class="hidden rounded-2xl border border-dashed border-white/10 px-6 py-14 text-center">
    ...
    <script>
        (() => {
            ...
            const counter = root.querySelector('[data-archive-count]');
            ...
            const apply = () => {
                ...
                groups.forEach((group) => {
                    const left = [...group.querySelectorAll('[data-archive-row]')].filter((row) => !row.classList.contains('hidden')).length;

                    group.querySelector('[data-year-count]').textContent = left + (left === 1 ? ' note' : ' notes');
                    group.classList.toggle('hidden', left === 0);
                });
