...
<div {{ $attributes->class('group/shell flex h-full w-full flex-col overflow-hidden bg-ink-950') }}>
    <div class="relative flex min-h-0 flex-1">
        <div data-ui-scroll-region data-blog-scroll class="flex min-w-0 flex-1 flex-col overflow-y-auto">
            <header class="sticky top-0 z-30 shrink-0 border-b border-white/5 bg-ink-950/85 backdrop-blur">
                <div class="mx-auto flex h-14 w-full max-w-5xl items-center gap-5 px-5 sm:px-8">
                    <a href="{{ route('templates.screen', ['blog', 'latest']) }}" target="_top" class="flex shrink-0 items-center gap-2.5">
                    ...
                    <nav class="hidden items-center gap-1 sm:flex">
                        @foreach ($links as $link)
                            <a href="{{ route('templates.screen', ['blog', $link['screen']]) }}" target="_top"
                ...
                @if ($progress)
                    <div class="h-px w-full bg-white/5">
                        <span data-read-progress class="block h-px w-0 bg-jade-500 transition-[width] duration-100 ease-linear"></span>
                    </div>
                @endif
            </header>

            <main class="mx-auto w-full max-w-5xl grow px-5 py-10 sm:px-8">{{ $slot }}</main>
            ...
            <footer class="border-t border-white/5 bg-ink-900/50">
                <div class="mx-auto flex w-full max-w-5xl flex-wrap items-center gap-x-5 gap-y-3 px-5 py-6 font-mono text-[10px] text-zinc-600 sm:px-8">
                    ...
                    <a href="{{ route('templates.screen', ['blog', 'archive']) }}" target="_top" class="transition-colors duration-150 hover:text-cream">Archive</a>
                    <a href="#" class="transition-colors duration-150 hover:text-cream">RSS</a>
                    <a href="{{ route('templates.screen', ['product', 'overview']) }}" target="_top" class="transition-colors duration-150 hover:text-cream">The shop</a>
                    <span class="ml-auto">22 notes · one every other Tuesday</span>
                </div>
            </footer>
        </div>

        <x-ui.scroll-top anchor="container" variant="progress" :threshold="400" />
    </div>
</div>
