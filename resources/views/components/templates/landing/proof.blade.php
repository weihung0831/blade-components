...
<x-templates.landing.shell active="The measurements" ribbon="Every figure below was measured in the workshop. The method is at the bottom of the page.">
    <div class="mx-auto max-w-5xl">
        ...
        <section data-proof class="mt-10">
            <div class="flex w-fit flex-wrap items-center gap-1 rounded-xl border border-white/8 bg-ink-900/60 p-1">
                @foreach ($sets as $set)
                    <button type="button" data-proof-tab="{{ $set['key'] }}"
                        @class([
                            'rounded-lg px-3 py-1.5 text-[13px] transition-colors duration-150 outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70',
                            'bg-white/8 text-cream' => $loop->first,
            ...
            @foreach ($sets as $set)
                <div data-proof-panel="{{ $set['key'] }}" @class(['mt-5', 'hidden' => ! $loop->first])>
                    ...
                    <div class="mt-6 flex flex-col gap-5 rounded-2xl border border-white/8 bg-ink-950 p-5">
                        @php $ceiling = max(array_column($set['rows'], 'value')); @endphp
        ...
        <section class="mt-14">
            ...
            <div class="mt-4 overflow-hidden rounded-2xl border border-white/8 bg-ink-950">
                @foreach ($drops as $drop)
                    <div @class(['flex flex-col gap-2 px-4 py-3.5 sm:flex-row sm:items-baseline sm:gap-5', 'border-t border-white/5' => ! $loop->first])>
                        ...
                        <span @class([
                            'inline-flex shrink-0 items-center gap-1.5 rounded-lg border px-2 py-0.5 font-mono text-[10px] uppercase',
                            'border-red-400/30 text-red-400' => $drop['tone'] === 'bad',
                            'border-amber-400/30 text-amber-300' => $drop['tone'] === 'warn',
                            'border-jade-500/30 text-jade-300' => $drop['tone'] === 'ok',
                        ])>{{ $drop['outcome'] }}</span>
                    </div>
                @endforeach
            </div>
        ...
        <section class="mt-12 grid grid-cols-1 gap-3 lg:grid-cols-3">
            <x-templates.landing.feature
                mark="method"
                title="Who measured it and with what"
                body="Sieve stack borrowed from the food science lab on Minsheng East Road, calibrated in January. Three runs per machine, the middle one reported. The bench sheets are scans of paper, handwriting and all."
                meta="ask for the scans, we will send them" />

            <x-templates.landing.feature
                mark="conflict"
                title="We paid for the machines we lost to"
                body="The electric and the aluminium grinder were bought at retail, by us, no discount and nobody told. Neither maker knows this page exists."
                meta="receipts on the same sheets"
                tone="caveat" />

            <x-templates.landing.feature
                mark="stale"
                title="What is out of date here"
                body="The spread test predates the batch 40 burr change, which our own runs say is worth a point or two. We will not claim it until the next full test in October."
                meta="next measurement, w/c 5 Oct"
                tone="primary" />
        </section>
        ...
        <section class="mt-12 rounded-2xl border border-white/8 bg-ink-900/50 p-5">
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3">
                <a href="{{ route('templates.screen', ['landing', 'objections']) }}" target="_top"
                    class="inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2.5 text-[13px] text-zinc-300 transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Five people who should buy the electric</a>
        ...
        (() => {
            ...
            const apply = (key) => {
                tabs.forEach((tab) => {
                    ...
                    tab.classList.toggle('bg-white/8', on);
                    tab.classList.toggle('text-cream', on);
                    tab.classList.toggle('text-zinc-500', !on);
                    tab.classList.toggle('hover:text-cream', !on);
                });

                panels.forEach((panel) => panel.classList.toggle('hidden', panel.dataset.proofPanel !== key));
            };

            tabs.forEach((tab) => tab.addEventListener('click', () => apply(tab.dataset.proofTab)));
        })();
    </script>
