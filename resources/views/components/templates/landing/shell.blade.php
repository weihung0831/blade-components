@props([
    'active' => 'The pitch',
    'ribbon' => 'Batch 41 opens 2 September. 46 of the 180 are still unspoken for.',
    'padded' => true,
])
...
@php
    ...
    $columns = [
        [
            'heading' => 'The machine',
            'items' => ['Mk3, NT$4,200', 'Burrs and spares', 'What is in the box', 'Every measurement we take'],
        ],
        [
            'heading' => 'The workshop',
            'items' => ['Where it is made', 'The four of us', 'Batch 39, eleven weeks late', 'What we stopped making'],
        ],
        [
            'heading' => 'If it goes wrong',
            'items' => ['Send it back, 60 days', 'The crank collar fix', 'Ana at the desk', 'Warranty, and what it covers'],
        ],
    ];
@endphp
...
<div {{ $attributes->class('flex h-full w-full flex-col overflow-hidden bg-ink-950') }}>
    ...
    <header class="shrink-0 border-b border-white/5 bg-ink-950">
        @if ($ribbon)
            <div class="flex items-center justify-center gap-2 border-b border-jade-500/15 bg-jade-500/6 px-4 py-1.5 text-center">
        ...
        <div class="flex h-14 items-center gap-5 px-4 sm:px-5">
            <a href="{{ route('templates.screen', ['landing', 'pitch']) }}" target="_top" class="flex shrink-0 items-center gap-2.5">
            ...
            <nav class="hidden items-center gap-1 lg:flex">
                @foreach ($links as $link)
                    <a href="{{ route('templates.screen', ['landing', $link['screen']]) }}" target="_top"
    ...
    <div class="relative flex min-h-0 flex-1 flex-col">
        @if ($padded)
            <main data-ui-scroll-region class="min-h-0 flex-1 overflow-y-auto">
                <div class="px-4 py-10 sm:px-5">{{ $slot }}</div>
