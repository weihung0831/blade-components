...
<x-templates.dashboard.shell active="Deploys" title="Deploys" :crumbs="[['label' => 'wharf', 'href' => '#'], ['label' => 'Platform', 'href' => '#'], ['label' => 'Deploys']]">
    <x-slot:actions>
        <x-ui.select :options="['Production', 'Staging', 'Preview']" value="Production" size="sm" name="deploy-env" class="w-36" />
        <x-ui.button variant="secondary" size="sm">Rollback</x-ui.button>
        ...
        @foreach ($services as $service)
            <div class="rounded-xl border border-white/10 bg-ink-800 p-4">
                <div class="flex items-center gap-2">
                    <span @class(['size-1.5 shrink-0 rounded-full', 'bg-jade-500' => $service['state'] === 'ok', 'bg-amber-400' => $service['state'] === 'degraded'])></span>
                ...
                <div class="mt-3 flex h-7 items-end gap-[3px]">
                    @foreach (range(0, 29) as $slot)
                        <span @class([
                            ...
                            'bg-red-400' => in_array($slot, $service['incidents'], true),
                            'bg-jade-500/35' => ! in_array($slot, $service['incidents'], true),
                        ])></span>
                    @endforeach
                </div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <x-ui.card class="lg:col-span-2">
            ...
            <x-ui.animated-column-chart :items="$latency" height="h-44" />
        </x-ui.card>
        ...
        <x-ui.card>
            ...
            <div class="flex flex-col gap-3.5">
                @foreach ($regions as $region)
                    <div>
                        ...
                        <x-ui.progress :value="$region['load']" size="sm" class="mt-1.5" />
                    </div>
                @endforeach
            </div>
            ...
            <x-ui.alert variant="warning" title="Webhook fanout degraded">
