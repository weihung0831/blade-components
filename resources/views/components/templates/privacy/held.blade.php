...
<x-templates.privacy.shell active="What we hold">
    <x-slot:toolbar>
        <div data-held-bar class="flex flex-wrap items-center gap-x-5 gap-y-2">
            ...
            <span data-held-count class="hidden font-mono text-[10px] text-zinc-700 sm:inline">showing all twenty-one</span>

            <div class="ml-auto flex flex-wrap items-center gap-1">
                @foreach ($filters as $filter)
                    <button type="button" data-held-filter="{{ $filter['key'] }}" @if ($loop->first) data-active @endif
    ...
    <div data-held class="mx-auto max-w-5xl">
        ...
        <div class="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <x-templates.privacy.clock
                label="Phone number after delivery"
                :span="18"
                unit="months"
                :elapsed="4"
                then="deleted without being asked" />

            <x-templates.privacy.clock
                label="The invoice, whatever anybody wants"
                :span="7"
                unit="years"
                :elapsed="2"
                then="deleted the week it clears"
                pinned />
        </div>

        <div class="mt-8 flex flex-col gap-8">
            @foreach ($groups as $group)
                <section id="purpose-{{ $group['key'] }}" data-held-group class="scroll-mt-4">
                    <div class="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                        <h2 class="text-[15px] font-medium tracking-tight text-cream">{{ $group['title'] }}</h2>
                        <span class="font-mono text-[10px] text-zinc-700">{{ count($group['fields']) }} fields</span>
                    </div>
        ...
        <p data-held-empty class="mt-4 hidden text-[12px]/5 text-zinc-600">Nothing under this filter.</p>
        ...
        <section class="mt-4 flex flex-wrap items-center gap-x-6 gap-y-3 rounded-xl border border-white/8 bg-ink-900 p-4">
            ...
            <a href="{{ route('templates.screen', ['privacy', 'request']) }}" target="_top"
                class="shrink-0 rounded-lg border border-white/12 px-3 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream">Ask for a copy of the lot</a>
        </section>
    </div>

    <script>
        (() => {
            const root = document.querySelector('[data-held]');
            const bar = document.querySelector('[data-held-bar]');

            if (!root || !bar) {
                return;
            }

            const groups = [...root.querySelectorAll('[data-held-group]')];
            const empty = root.querySelector('[data-held-empty]');
            const count = bar.querySelector('[data-held-count]');
            const buttons = [...bar.querySelectorAll('[data-held-filter]')];

            const spell = ['none', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten',
            ...
            const apply = (key) => {
                ...
                groups.forEach((group) => {
                    ...
                    group.querySelectorAll('[data-field]').forEach((field) => {
                        const keep = key === 'all' || field.dataset.removable === key;

                        field.classList.toggle('hidden', !keep);
                        kept += keep ? 1 : 0;
                    });

                    group.classList.toggle('hidden', kept === 0);
                    shown += kept;
                });

                buttons.forEach((button) => button.toggleAttribute('data-active', button.dataset.heldFilter === key));
                empty.classList.toggle('hidden', shown > 0);
                count.textContent = key === 'all' ? 'showing all twenty-one' : `showing ${spell[shown]} of twenty-one`;
            };

            buttons.forEach((button) => button.addEventListener('click', () => apply(button.dataset.heldFilter)));
        })();
    </script>
