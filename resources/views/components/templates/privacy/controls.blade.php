...
<x-templates.privacy.shell active="Your switches">
    <x-slot:toolbar>
        <div data-controls-bar class="flex flex-wrap items-center gap-x-5 gap-y-2">
            <span data-controls-count class="font-mono text-[10px] text-zinc-500">1 of 4 on</span>

            <span class="hidden font-mono text-[10px] text-zinc-700 sm:inline">nothing here is on unless you turned it on</span>

            <div class="ml-auto flex items-center gap-1.5">
                <button type="button" data-controls-all="off"
                    class="rounded-lg border border-white/12 px-2.5 py-1 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-white/25 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Turn it all off</button>
                <button type="button" data-controls-all="on"
                    class="rounded-lg border border-white/12 px-2.5 py-1 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-white/25 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Turn it all on</button>
            </div>
        </div>
    </x-slot:toolbar>

    <div data-controls class="mx-auto max-w-4xl">

        <h1 class="text-lg font-semibold tracking-tight text-cream">Four switches, and what each one breaks</h1>
        ...
        <div class="mt-6 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
        ...
        <section class="mt-9">
            <h2 class="text-[15px] font-medium tracking-tight text-cream">The bar itself</h2>
            ...
            <div class="mt-3 rounded-xl border border-white/8 bg-ink-800 p-4">
                <div class="flex flex-col gap-3.5 sm:flex-row sm:items-center sm:gap-5">
                    <div class="min-w-0 flex-1">
                        <p class="text-[13px] text-cream">Three optional things, off right now</p>
                        <p class="mt-1 text-[12px]/5 text-zinc-500">
                            ...
                            <a href="#" class="text-jade-400 transition-colors duration-150 hover:text-jade-300">Pick them one at a time</a>
                            if you would rather.
                        </p>
                    </div>
                    <div class="flex shrink-0 gap-2">
                        <button type="button" class="rounded-lg border border-white/15 px-3.5 py-1.5 text-[13px] text-zinc-200 transition-colors duration-150 outline-none hover:border-white/30 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">No thanks</button>
                        <button type="button" class="rounded-lg bg-jade-500 px-3.5 py-1.5 text-[13px] font-medium text-ink-950 transition-colors duration-150 outline-none hover:bg-jade-400 focus-visible:ring-2 focus-visible:ring-jade-500/70">All three, fine</button>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="mt-9">
            <h2 class="text-[15px] font-medium tracking-tight text-cream">What the switches do not reach</h2>
            <p class="mt-1 max-w-2xl text-[12px]/5 text-zinc-500">
            ...
            <div class="mt-3 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                @foreach ($outside as $row)
                    <div class="flex flex-col gap-1.5 px-4 py-3 sm:flex-row sm:items-baseline sm:gap-5">
                        <p class="w-full shrink-0 text-[13px] text-cream sm:w-44">{{ $row['name'] }}</p>
                        <p class="min-w-0 flex-1 text-[12px]/5 text-zinc-500">{{ $row['why'] }}</p>
                        <p class="shrink-0 font-mono text-[10px] text-zinc-600">{{ $row['basis'] }}</p>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        <section class="mt-4 flex flex-wrap items-center gap-x-6 gap-y-3 rounded-xl border border-white/8 bg-ink-900 p-4">
            <div class="min-w-0 flex-1">
                <p class="text-[13px] text-cream">Turning something off does not delete what it already collected</p>
                <p class="mt-1 text-[12px]/5 text-zinc-500">
            ...
            <a href="{{ route('templates.screen', ['privacy', 'request']) }}" target="_top"
                class="shrink-0 rounded-lg border border-white/12 px-3 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream">Ask us to erase it</a>
        </section>
    </div>
    ...
    <script>
        (() => {
            ...
            const bar = document.querySelector('[data-controls-bar]');

            if (!root || !bar) {
                return;
            }

            const sections = [...root.querySelectorAll('[data-consent]')];
            const count = bar.querySelector('[data-controls-count]');

            const sync = () => {
                let on = 0;

                sections.forEach((section) => {
                    const input = section.querySelector('[data-consent-input]');
                    const state = section.querySelector('[data-consent-state]');

                    section.toggleAttribute('data-on', input.checked);
                    on += input.checked ? 1 : 0;

                    if (state) {
                        state.textContent = input.checked ? 'on' : 'off';
                        state.classList.toggle('text-jade-400/90', input.checked);
                        state.classList.toggle('text-zinc-600', !input.checked);
                    }
                });

                count.textContent = `${on} of ${sections.length} on`;
            };

            sections.forEach((section) => section.querySelector('[data-consent-input]').addEventListener('change', sync));

            bar.querySelectorAll('[data-controls-all]').forEach((button) => {
                button.addEventListener('click', () => {
                    const wanted = button.dataset.controlsAll === 'on';
