...
<div {{ $attributes->class('group/shell flex h-full w-full overflow-hidden bg-ink-950') }} data-range="28d">
    <nav class="hidden w-14 shrink-0 flex-col items-center gap-1 border-r border-white/5 bg-ink-900/60 py-3 md:flex">
        <a href="{{ route('templates.screen', ['analytics', 'explore']) }}" target="_top"
        ...
        @foreach ($tabs as $tab)
            <a href="{{ route('templates.screen', ['analytics', $tab['screen']]) }}" target="_top"
                @if ($tab['label'] === $active) aria-current="page" @endif
                @class([
                    'group/rail relative grid size-9 place-items-center rounded-lg transition-colors duration-150 outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70',
                    'bg-jade-500/12 text-jade-300' => $tab['label'] === $active,
                    'text-zinc-600 hover:bg-white/5 hover:text-cream' => $tab['label'] !== $active,
                ])>
        ...
        <x-ui.avatar initials="WH" size="sm" color="jade" class="mt-3" />
    </nav>
    ...
    <div class="relative flex min-w-0 flex-1">
        <div data-ui-scroll-region class="flex min-w-0 flex-1 flex-col overflow-y-auto">
            <header class="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-3 border-b border-white/5 bg-ink-950/85 px-4 backdrop-blur sm:px-6">
                <x-ui.dropdown variant="ghost" class="shrink-0 [&>summary]:h-8 [&>summary]:px-2 [&>summary]:text-[13px]">
                ...
                <div class="ml-auto flex shrink-0 items-center gap-2 sm:gap-3">
                    <span class="hidden font-mono text-[10px] text-zinc-600 lg:inline">
                        <span class="hidden group-data-[range=7d]/shell:inline">11 – 17 Aug 2026</span>
                        <span class="hidden group-data-[range=28d]/shell:inline">21 Jul – 17 Aug 2026</span>
                        <span class="hidden group-data-[range=90d]/shell:inline">20 May – 17 Aug 2026</span>
                    </span>
                    ...
                    <div class="inline-flex items-center rounded-lg border border-white/10 bg-ink-900 p-0.5 font-mono text-[11px]">
                        <button type="button" data-range-set="7d"
                            class="rounded-md px-2 py-1 text-zinc-500 transition-colors duration-150 outline-none group-data-[range=7d]/shell:bg-jade-500/15 group-data-[range=7d]/shell:text-jade-300 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">7d</button>
                        <button type="button" data-range-set="28d"
                            class="rounded-md px-2 py-1 text-zinc-500 transition-colors duration-150 outline-none group-data-[range=28d]/shell:bg-jade-500/15 group-data-[range=28d]/shell:text-jade-300 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">28d</button>
                        <button type="button" data-range-set="90d"
                            class="rounded-md px-2 py-1 text-zinc-500 transition-colors duration-150 outline-none group-data-[range=90d]/shell:bg-jade-500/15 group-data-[range=90d]/shell:text-jade-300 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">90d</button>
                    </div>

                    <x-ui.button variant="secondary" size="sm" class="hidden sm:inline-flex">Share</x-ui.button>
                    ...
                    <x-ui.dropdown variant="ghost" align="right" class="md:hidden [&>summary]:h-8 [&>summary]:px-2.5 [&>summary]:text-[13px]">
                        ...
                        <x-slot:menu>
                            @foreach ($tabs as $tab)
                                <a href="{{ route('templates.screen', ['analytics', $tab['screen']]) }}" target="_top"
                                    @class(['text-jade-300!' => $tab['label'] === $active])>{{ $tab['label'] }}</a>
                            @endforeach

                            <hr>

                            <a href="{{ route('templates.screen', ['dashboard', 'overview'])  }}" target="_top">Back to console</a>
                        </x-slot>
                    </x-ui.dropdown>
                </div>
            </header>
            ...
            <div class="border-b border-white/5 px-4 py-6 sm:px-6">
                <div class="flex flex-wrap items-end justify-between gap-x-8 gap-y-4">
                    <div>
                        ...
                        @if ($description)
                            <p class="mt-2 max-w-xl text-[13px]/6 text-zinc-500">{{ $description }}</p>
                        @endif
                    </div>
            ...
            <main class="flex grow flex-col gap-5 px-4 py-6 sm:px-6">{{ $slot }}</main>
