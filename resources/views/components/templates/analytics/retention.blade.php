...
<x-templates.analytics.shell active="Retention" title="Who comes back, and how long the first order buys you"
    ...
    <x-slot:actions>
        <x-ui.select :options="['order_paid', 'session_start', 'add_to_cart']" value="order_paid" size="sm" name="retention-event" class="w-40" />
        <x-ui.button variant="secondary" size="sm">Compare plans</x-ui.button>
    </x-slot>

    <x-slot:toolbar>
        <div class="flex flex-wrap items-center gap-2">
            <x-templates.analytics.token type="event" label="order_paid" value="first occurrence" :removable="false" />
            <x-templates.analytics.token type="return" label="order_paid" value="any later occurrence" :removable="false" />
            <x-templates.analytics.token type="by" label="cohort" value="follows the range" :removable="false" />
        </div>
    </x-slot>

    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <x-templates.analytics.metric label="Day 1"
        ...
        <x-templates.analytics.metric label="Day 7"
        ...
        <x-templates.analytics.metric label="Day 30"
        ...
        <x-templates.analytics.metric label="Still active"
    ...
    <x-ui.card>
        <x-slot:header>
            <div class="flex flex-wrap items-baseline justify-between gap-3">
                <div>
                    <h2 class="text-sm font-medium text-cream">Retention grid</h2>
                    <p class="mt-0.5 text-xs text-zinc-500">
                        <span class="{{ $show['7d'] }}">Daily cohorts, seven of them, still filling in.</span>
                        <span class="{{ $show['28d'] }}">Weekly cohorts. The newest row has one day of data.</span>
                        <span class="{{ $show['90d'] }}">Monthly cohorts. May is the only one with a full d90.</span>
                    </p>
                </div>
                <span class="font-mono text-[11px] text-zinc-600">dashed cells have not happened yet</span>
            </div>
        </x-slot>

        @foreach (['7d', '28d', '90d'] as $range)
            <x-templates.analytics.cohort :columns="$grids[$range]['columns']" :rows="$grids[$range]['rows']" class="{{ $block[$range] }}" />
        @endforeach
    </x-ui.card>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <x-ui.card class="lg:col-span-2">
            <x-slot:header>
                <div class="flex flex-wrap items-baseline justify-between gap-3">
                    <h2 class="text-sm font-medium text-cream">Retention curve</h2>
                    <span class="font-mono text-[11px] text-zinc-600">
                        <span class="{{ $show['7d'] }}">flattens around d5</span>
                        <span class="{{ $show['28d'] }}">flattens around d14</span>
                        <span class="{{ $show['90d'] }}">still sliding at d90</span>
                    </span>
                </div>
            </x-slot>

            <x-templates.analytics.series :series="$curve" :axis="$axis" :scale="$scale" height="h-56" />
        </x-ui.card>

        <x-ui.card>
            <x-slot:header>
                <div class="flex items-baseline justify-between">
                    <h2 class="text-sm font-medium text-cream">Day 30 by plan</h2>
                    <span class="font-mono text-[11px] text-zinc-600">28d cohorts</span>
                </div>
            </x-slot>
                ...
                @foreach ($plans as $plan)
                    <li>
                        <div class="flex items-baseline justify-between gap-3">
                            <span class="text-[13px] text-zinc-300">{{ $plan['label'] }}</span>
                            <span class="font-mono text-[11px] text-cream">{{ $plan['value'] }}%</span>
                        </div>
                        <div class="mt-1.5 h-1.5 overflow-hidden rounded-full bg-ink-950">
                            <span @class([
                                'block h-full rounded-full',
                                'bg-jade-500' => $plan['value'] >= 50,
                                'bg-jade-500/50' => $plan['value'] < 50 && $plan['value'] >= 20,
                                'bg-white/15' => $plan['value'] < 20,
                            ]) style="width: {{ $plan['value'] }}%"></span>
                        </div>
                        <p class="mt-1.5 font-mono text-[10px] text-zinc-700">{{ $plan['note'] }}</p>
                    </li>
                @endforeach
            </ul>
        </x-ui.card>
    ...
    <section class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div class="rounded-xl border border-white/8 bg-ink-900 p-5">
            <p class="font-mono text-[10px] tracking-wider text-jade-400 uppercase">How a cell is counted</p>
            <p class="mt-3 text-[13px]/6 text-zinc-400">
                A user lands in the cohort of the day their first <span class="font-mono text-[12px] text-zinc-300">order_paid</span> cleared.
        ...
        <div class="rounded-xl border border-white/8 bg-ink-900 p-5">
            <p class="font-mono text-[10px] tracking-wider text-jade-400 uppercase">Reading the grid</p>
            <p class="mt-3 text-[13px]/6 text-zinc-400">
