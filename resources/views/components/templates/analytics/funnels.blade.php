...
<x-templates.analytics.shell active="Funnels" title="Five steps, and the two that cost you the order"
    ...
    <x-slot:actions>
        <x-ui.select :options="['30 minute window', '1 hour window', '24 hour window', '7 day window']" value="30 minute window" size="sm" name="funnel-window" class="w-44" />
        <x-ui.button variant="secondary" size="sm">Export CSV</x-ui.button>
    </x-slot>

    <x-slot:toolbar>
        <div class="flex flex-wrap items-center gap-2">
            @foreach ($steps['28d'] as $step)
                @if (! $loop->first)
                    <svg class="size-3 shrink-0 text-zinc-700" viewBox="0 0 12 12" fill="none"><path d="M4 2.5 7.5 6 4 9.5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
                @endif

                <span class="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-ink-900 px-2.5 py-1.5">
                    <span class="font-mono text-[10px] text-zinc-700">{{ sprintf('%02d', $loop->iteration) }}</span>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <x-templates.analytics.metric label="Median time to convert"
    ...
    <x-ui.card>
        ...
        @foreach ($ranges as $range)
            <ol class="{{ $block[$range] }}">
                @foreach ($steps[$range] as $step)
                    <li>
                        @if ($step['gap'])
                            <div class="flex items-center gap-3 py-2 pl-1">
                                ...
                                <span @class([
                                    'font-mono text-[10px]',
                                    'text-red-400' => $step['event'] === 'add_to_cart',
                                    'text-zinc-600' => $step['event'] !== 'add_to_cart',
                                ])>{{ $step['drop'] }} left here</span>
                            </div>
                        @endif
                        ...
                        <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                            ...
                            <span class="flex items-baseline gap-3 font-mono text-[11px]">
                                ...
                                <span class="w-14 text-right text-zinc-500">{{ number_format($step['percent'], 1) }}%</span>
                            </span>
                        </div>

                        <div class="mt-2 h-9 overflow-hidden rounded-lg bg-ink-950">
                            <span @class([
                                'block h-full rounded-lg transition-[width] duration-700 ease-snap',
                                'bg-jade-500' => $loop->last,
                                'bg-jade-500/70' => ! $loop->last,
                            ]) style="width: {{ max($step['percent'], 4) }}%"></span>
                        </div>
                    </li>
                @endforeach
            </ol>
                ...
                <tbody>
                    @foreach ($segments as $segment)
                        <tr>
                            ...
                            <td @class(['border-b border-white/5 py-3 text-right align-top font-mono text-[11px]', 'text-jade-400' => $segment['up'], 'text-red-400' => ! $segment['up']])>{{ $segment['delta'] }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <x-slot:header>
                <div class="flex flex-wrap items-baseline justify-between gap-3">
                    ...
                    <span class="font-mono text-[11px] text-zinc-600">
                        <span class="{{ $show['7d'] }}">7,964 dropped at checkout</span>
                        <span class="{{ $show['28d'] }}">30,460 dropped at checkout</span>
                        <span class="{{ $show['90d'] }}">85,790 dropped at checkout</span>
                    </span>
                </div>
            </x-slot>

            <ul class="flex flex-col gap-3.5">
                @foreach ($instead as $item)
                    <li>
