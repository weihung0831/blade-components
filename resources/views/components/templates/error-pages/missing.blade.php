@php
    $moved = [
        ...
        [
            'address' => '/shop/grinders/nomad-hand-grinder-mk3',
            'was' => 'Mk3 hand grinder — NT$4,200',
            'happened' => 'The one the Mk2 turned into. Same 38mm body, a burr set three suppliers make, and it takes the Mk2 crank if you kept yours.',
            'now' => 'In stock, ships from Taipei tomorrow',
            'when' => 'live since Nov 2024',
        ],
        [
        ...
        [
            'address' => '/support/mk2-crank-wobble',
            'was' => 'The Mk2 crank wobble, and the two minutes it takes to fix',
            'happened' => 'Kept alive on purpose. There are 6,000 Mk2s out there and the collar works loose on all of them eventually.',
            'now' => 'Read the fix',
            'when' => 'still here',
        ],
        [
...
<x-templates.error-pages.shell active="Nothing here" state="ok" reference="req_9f4c21a8 · 404">
    <x-slot:toolbar>
        <div class="mx-auto flex max-w-3xl items-center gap-3">
            <label class="flex min-w-0 flex-1 items-center gap-2 rounded-lg border border-white/10 bg-ink-900 px-2.5 py-1.5 focus-within:border-jade-500/60">
                ...
                <input type="search" data-moved-search placeholder="What were you after? Try &quot;burr&quot; or &quot;mk2&quot;"
                    class="min-w-0 flex-1 bg-transparent text-[13px] text-cream outline-none placeholder:text-zinc-700">
            </label>

            <span data-moved-count class="shrink-0 font-mono text-[10px] text-zinc-600">4 of 4</span>
        </div>
    </x-slot:toolbar>
    ...
    <div class="mx-auto max-w-3xl">
        ...
        <section data-moved-list class="mt-8">
            <div class="flex items-baseline gap-3">
            ...
            <div class="mt-3 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-950">
                @foreach (array_slice($moved, 1) as $entry)
                    <x-templates.error-pages.moved
            ...
            <p data-moved-empty class="mt-3 hidden rounded-xl border border-white/8 bg-ink-900 px-3.5 py-8 text-center text-[12px] text-zinc-600">
        ...
        <section class="mt-8 rounded-xl border border-white/8 bg-ink-900 p-4">
            ...
            <div class="mt-3 flex flex-wrap items-center gap-2">
                <button type="button"
                    class="rounded-lg border border-white/10 px-2.5 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Report the link that sent me</button>
    ...
    <script>
        (() => {
            const search = document.querySelector('[data-moved-search]');
            const list = document.querySelector('[data-moved-list]');

            if (!search || !list) {
                return;
            }

            const rows = [...list.querySelectorAll('[data-moved]')];
            const count = document.querySelector('[data-moved-count]');
            const empty = list.querySelector('[data-moved-empty]');
            ...
            const apply = () => {
                ...
                rows.forEach((row) => {
                    const on = words.every((word) => row.dataset.terms.includes(word));
