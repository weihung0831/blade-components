@php
    $services = [
        ...
        ['name' => 'The shop', 'state' => 'normal', 'means' => 'Browsing, search and the basket are all fine. You can keep filling it.', 'since' => null],
        ['name' => 'Your account', 'state' => 'normal', 'means' => 'Orders, addresses and past invoices all read normally.', 'since' => null],
        ['name' => 'Order emails', 'state' => 'slow', 'means' => 'Queued rather than lost. They go out in order once checkout is back.', 'since' => '6 min'],
        ['name' => 'The desk', 'state' => 'normal', 'means' => 'Ana is at it and already knows about this one.', 'since' => null],
    ];
...
<x-templates.error-pages.shell active="Our fault" state="down" reference="req_2c81f0d3 · 500 · 04:19 GMT+8">
    <x-slot:toolbar>
        <div class="mx-auto flex max-w-3xl flex-wrap items-center gap-x-4 gap-y-1.5">
            ...
            <span class="text-[11px] text-zinc-500">41 shops, 6 orders caught mid-payment, none of them charged</span>
            <span class="ml-auto font-mono text-[10px] text-zinc-600">next update 04:35, whether or not it is fixed</span>
        </div>
    </x-slot:toolbar>
    ...
    <div class="mx-auto max-w-3xl">
        ...
        <section class="mt-8 overflow-hidden rounded-xl border border-jade-500/25 bg-jade-500/4">
            <div class="border-b border-jade-500/15 px-4 py-3">
                <p class="font-mono text-[10px] tracking-wider text-jade-300 uppercase">The answer to the question you came with</p>
                <p class="mt-1.5 text-[15px] text-cream">Order NS-24817 is held, and the card has not been touched.</p>
            </div>

            <dl class="grid grid-cols-1 divide-y divide-white/5 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
                <div class="px-4 py-3">
                    <dt class="font-mono text-[10px] text-zinc-600">What we have</dt>
                    <dd class="mt-1 text-[13px]/5 text-zinc-300">One Mk3 grinder and a spare burr set, NT$4,980, going to the Da'an address.</dd>
                </div>
                <div class="px-4 py-3">
                    <dt class="font-mono text-[10px] text-zinc-600">The card</dt>
                    <dd class="mt-1 text-[13px]/5 text-zinc-300">Authorised, never captured. The hold falls off by itself inside seven days if we never finish this.</dd>
                </div>
                <div class="px-4 py-3">
                    <dt class="font-mono text-[10px] text-zinc-600">What happens</dt>
                    <dd class="mt-1 text-[13px]/5 text-zinc-300">Checkout comes back, we finish it, and you get the confirmation that should have been this page.</dd>
                </div>
            </dl>

            <div class="flex flex-wrap items-center gap-2 border-t border-jade-500/15 px-4 py-3">
                <a href="#" target="_top"
                    class="rounded-lg border border-jade-500/40 bg-jade-500/10 px-2.5 py-1.5 text-[12px] text-cream transition-colors duration-150 hover:border-jade-500/70">Watch NS-24817</a>

                <button type="button" data-notify
                    class="rounded-lg border border-white/10 px-2.5 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">
                    <span data-notify-label>Mail me when it is finished</span>
                </button>

                <span data-notify-note class="font-mono text-[10px] text-zinc-600">one mail, to the address on the order</span>
            </div>
        </section>
        ...
        <div class="mt-8 grid grid-cols-1 gap-4 lg:grid-cols-2">
            <section>
                <div class="flex items-baseline gap-3">
                    <h2 class="font-mono text-[11px] tracking-wider text-zinc-600 uppercase">What is still working</h2>
            ...
            <section>
                <div class="flex items-baseline gap-3">
                    <h2 class="font-mono text-[11px] tracking-wider text-zinc-600 uppercase">What we are doing about it</h2>
                    <span class="h-px min-w-0 flex-1 bg-white/6"></span>
                </div>

                <div class="mt-3 rounded-xl border border-white/8 bg-ink-950 p-3.5">
                            ...
                            <li class="flex gap-3">
                                <span class="flex flex-col items-center pt-1">
                                    <span @class([
                                        ...
                                        'bg-red-400' => $update['state'] === 'now',
                                        'bg-white/25' => $update['state'] !== 'now',
                                    ])></span>
                                    @if (! $loop->last)
                                ...
                                <span class="min-w-0 flex-1 pb-1">
                                    <span class="flex items-baseline gap-2">
                                        <span class="font-mono text-[11px] text-zinc-400">{{ $update['time'] }}</span>
                                        <span class="font-mono text-[10px] text-zinc-700">{{ $update['who'] }}</span>
                                    </span>
                                    <span class="mt-1 block text-[12px]/5 text-zinc-400">{{ $update['text'] }}</span>
                                </span>
                            </li>
                        @endforeach
                    </ol>
    ...
    <script>
        (() => {
            ...
            const label = button.querySelector('[data-notify-label]');
            const note = document.querySelector('[data-notify-note]');
            let on = false;

            button.addEventListener('click', () => {
                on = !on;

                label.textContent = on ? 'We will mail you' : 'Mail me when it is finished';
                note.textContent = on ? 'goes to the address on NS-24817, once, and that is the end of it' : 'one mail, to the address on the order';
