...
<div {{ $attributes->merge(['class' => 'relative']) }}>
    ...
    <pre class="overflow-x-auto p-4 font-mono text-[13px]/6 text-zinc-400"><code>@foreach ($lines as $line)<span class="mr-4 inline-block {{ $gutterWidth }} text-right text-zinc-700 select-none">{{ $loop->iteration }}</span>{!! $lang === 'css' ? App\Support\BladeSyntaxHighlighter::highlightCss($line) : App\Support\BladeSyntaxHighlighter::highlight($line) !!}
@endforeach</code></pre>
</div>
