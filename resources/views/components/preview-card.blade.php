...
<{{ $tag }} {{ $attributes->merge(['class' => 'rise group overflow-hidden rounded-xl border border-white/8 bg-ink-900 transition-colors duration-150'.($soon ? '' : ' hover:border-jade-500/60')]) }}>
    ...
    <div class="flex items-center justify-between gap-2 px-4 py-3">
        <h3 class="truncate text-sm font-medium {{ $soon ? 'text-zinc-500' : 'text-cream' }}">{{ $name }}</h3>
        @if ($soon)
        ...
        @else
            <span class="shrink-0 font-mono text-xs text-zinc-600">{{ sprintf('%02d', $variants) }} variants</span>
        @endif
    </div>
</{{ $tag }}>
