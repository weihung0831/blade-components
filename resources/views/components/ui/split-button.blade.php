...
<div {{ $attributes->class(['relative inline-flex', 'pointer-events-none opacity-40' => $disabled])->merge(['role' => 'group']) }}>
    <button type="button" @disabled($disabled) class="{{ $buttonBase }} h-10 px-5 text-sm font-medium {{ $styles['main'] }}">{{ $slot }}</button>
    <details class="group/menu" name="ui-split-button-menu">
        <summary aria-label="More options" class="{{ $buttonBase }} grid h-10 w-9 cursor-pointer list-none place-items-center [&::-webkit-details-marker]:hidden group-open/menu:before:fixed group-open/menu:before:inset-0 group-open/menu:before:cursor-default group-open/menu:before:content-[''] {{ $styles['caret'] }}">
            <svg class="size-3.5 transition-transform duration-150 ease-snap group-open/menu:rotate-180" viewBox="0 0 16 16" fill="none"><path d="m4 6 4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </summary>
        @isset($menu)
