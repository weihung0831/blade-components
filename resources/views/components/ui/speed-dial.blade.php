...
@php
    ...
    $style = $directions[$direction] ?? $directions['up'];
@endphp

<div {{ $attributes->merge(['class' => 'group inline-flex items-center gap-2.5 '.$style['container']]) }}>
    <button type="button" aria-label="{{ $label }}" aria-haspopup="true"
    ...
    <div class="pointer-events-none flex items-center gap-2.5 opacity-0 transition-[opacity,translate] duration-200 ease-snap {{ $style['container'] }} {{ $style['shift'] }} group-focus-within:pointer-events-auto group-focus-within:translate-x-0 group-focus-within:translate-y-0 group-focus-within:opacity-100 group-hover:pointer-events-auto group-hover:translate-x-0 group-hover:translate-y-0 group-hover:opacity-100">
        {{ $slot }}
    </div>
</div>
