...
const layout = computed(() => layouts[props.orientation] ?? layouts.horizontal);
</script>

<template>
    <nav :class="[layout.root, 'rounded-2xl border border-white/10 bg-ink-800/70 p-2 backdrop-blur']">
        <a
        ...
        >
            <span
                class="relative grid size-11 place-items-center rounded-xl transition-transform duration-200 ease-snap"
                :class="[layout.tile, entry.active ? 'bg-jade-500 text-ink-950' : 'bg-ink-950 text-zinc-400 group-hover/dock:text-cream']"
            >
                <svg class="size-5" viewBox="0 0 16 16" fill="none"><path :d="icons[entry.icon] ?? icons.dot" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>

                <span v-if="entry.badge" class="absolute -top-1 -right-1 grid min-w-4 place-items-center rounded-full bg-red-400 px-1 font-mono text-[9px] text-ink-950">{{ entry.badge }}</span>
            </span>

            <span
                aria-hidden="true"
                class="pointer-events-none absolute z-10 rounded-md border border-white/10 bg-ink-900 px-2 py-1 text-xs whitespace-nowrap text-zinc-300 opacity-0 shadow-lg shadow-black/40 transition-opacity duration-150 group-hover/dock:opacity-100 group-focus-visible/dock:opacity-100"
                :class="layout.tip"
            >{{ entry.label }}</span>
        </a>
    </nav>
