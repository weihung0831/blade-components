...
<template>
    <div class="relative flex items-center gap-1 rounded-xl border border-white/10 bg-ink-800 px-2 py-1.5">
        <div v-if="$slots.brand" class="mr-1 flex items-center"><slot name="brand" /></div>

        <details v-for="menu in menus" :key="menu.label" class="group/mega static sm:relative" name="ui-mega-menu">
            <summary
                :class="`flex cursor-pointer list-none items-center gap-1.5 rounded-md px-2.5 py-1 text-sm text-zinc-400 transition-colors duration-150 outline-none select-none hover:bg-white/5 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70 [&::-webkit-details-marker]:hidden group-open/mega:bg-white/8 group-open/mega:text-cream group-open/mega:before:fixed group-open/mega:before:inset-0 group-open/mega:before:cursor-default group-open/mega:before:content-['']`"
            >
                {{ menu.label }}
            ...
            <div class="absolute top-full left-0 z-10 mt-1.5 w-full rounded-xl border border-white/10 bg-ink-900 p-5 shadow-xl shadow-black/50 sm:w-max sm:max-w-[min(42rem,90vw)]">
                <div class="grid gap-6" :class="gridClasses(menu)">
                    <div v-for="column in menu.columns ?? []" :key="column.title" class="sm:w-48">
                        ...
                        <div class="mt-2.5 flex flex-col gap-0.5">
                            <a
                                ...
                                :key="entry.label"
                                :href="entry.href ?? '#'"
                                class="-mx-2 rounded-md px-2 py-1.5 transition-colors duration-150 outline-none hover:bg-white/5 focus-visible:ring-2 focus-visible:ring-jade-500/70"
                            >
                                <span class="block text-sm text-zinc-200">{{ entry.label }}</span>
                                <span v-if="entry.description" class="mt-0.5 block text-xs/5 text-zinc-500">{{ entry.description }}</span>
                            </a>
                        </div>
                    </div>

                    <div v-if="menu.feature" class="flex flex-col justify-between rounded-lg border border-jade-500/25 bg-jade-500/8 p-4 sm:w-48">
                        <div>
                        ...
                        <a
                            ...
                            class="mt-4 inline-flex items-center gap-1.5 rounded text-xs font-medium text-jade-400 transition-colors duration-150 outline-none hover:text-jade-300 focus-visible:ring-2 focus-visible:ring-jade-500/70"
                        >
                            {{ menu.feature.action }}
