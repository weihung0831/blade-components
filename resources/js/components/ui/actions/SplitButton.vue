...
const groupClasses = computed(() =>
    ['relative inline-flex', props.disabled ? 'pointer-events-none opacity-40' : ''].filter(Boolean).join(' '),
);
</script>

<template>
    <div role="group" :class="groupClasses">
        <button type="button" :disabled="disabled" :class="[buttonBase, 'h-10 px-5 text-sm font-medium', styles.main]">
            <slot />
        </button>
        <details class="group/menu" name="ui-split-button-menu">
            <summary
                aria-label="More options"
                :class="[buttonBase, `grid h-10 w-9 cursor-pointer list-none place-items-center [&::-webkit-details-marker]:hidden group-open/menu:before:fixed group-open/menu:before:inset-0 group-open/menu:before:cursor-default group-open/menu:before:content-['']`, styles.caret]"
            >
            ...
            <div v-if="slots.menu" role="menu" :class="menuClasses">
                <slot name="menu" />
            </div>
        </details>
    </div>
