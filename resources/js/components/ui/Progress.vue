...
<template>
    <div class="w-full">
        <div v-if="label" class="mb-2 flex items-baseline justify-between gap-4">
            ...
            <span v-if="!indeterminate" class="font-mono text-xs text-jade-400">{{ Math.round(percent) }}%</span>
        </div>
        <div
            role="progressbar"
            aria-valuemin="0"
        ...
        >
            ...
            <div v-else class="h-full rounded-full bg-jade-500 transition-[width] duration-500 ease-snap" :style="{ width: percent + '%' }"></div>
        </div>
    </div>
</template>
