...
const costed = computed(() => machines.map((machine) => ({
    ...machine,
    cost: machine.costs[period.value],
    display: `NT$${machine.costs[period.value].toLocaleString('en-US')}`,
})));
</script>
            ...
            <LandingHero
                ...
                <div class="flex h-full flex-col rounded-2xl border border-white/8 bg-ink-900/70 p-4">
                    ...
                    <dl class="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 border-t border-white/6 pt-3">
                        <div v-for="row in spec" :key="row[0]" class="flex items-baseline justify-between gap-2">
                ...
                <div class="flex flex-wrap items-end justify-between gap-4">
                    ...
                    <div class="flex shrink-0 items-center gap-1 rounded-xl border border-white/8 bg-ink-950 p-1">
                        <button
                            ...
                            :class="period === option.key ? 'bg-jade-500/15 text-jade-300' : 'text-zinc-500 hover:text-cream'"
                ...
                <div class="flex flex-col gap-6 p-6 sm:flex-row sm:items-center">
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="mt-4 flex flex-wrap items-center gap-3">
                            <a href="/templates/landing/screens/batch" target="_top" class="inline-flex items-center gap-2 rounded-xl bg-jade-500 px-4 py-2.5 text-[13px] font-medium text-ink-950 transition-colors duration-150 outline-none hover:bg-jade-400 focus-visible:ring-2 focus-visible:ring-jade-500/70">Take a place</a>
                    ...
                    <div class="w-full shrink-0 rounded-xl border border-white/8 bg-ink-950/60 p-4 sm:w-64">
                        ...
                        <dl class="mt-4 flex flex-col gap-2 border-t border-white/6 pt-3">
                            <div v-for="row in shipping" :key="row[0]" class="flex items-baseline justify-between gap-2">
