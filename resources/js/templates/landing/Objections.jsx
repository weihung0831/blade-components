...
import { LandingObjection } from './Objection';
...
export function LandingObjections() {
    const [sort, setSort] = useState('heard');
        ...
        <LandingShell active="Not for you" ribbon="This page exists because 78 machines came back from people who thought it would be faster.">
            <div className="mx-auto max-w-5xl">
                <header className="max-w-2xl">
                    ...
                    <h1 className="mt-4 text-3xl leading-[1.15] font-semibold tracking-tight text-balance text-cream">Five people who should buy something else, and what they should buy.</h1>
                ...
                <section className="mt-10">
                    <div className="flex flex-wrap items-baseline justify-between gap-3">
                        <h2 className="font-mono text-[11px] tracking-wider text-zinc-600 uppercase">{HEADINGS[sort]}</h2>

                        <div className="flex shrink-0 items-center gap-1 rounded-xl border border-white/8 bg-ink-900/60 p-1">
                            <button
                                type="button"
                                onClick={() => setSort('heard')}
                                className={`rounded-lg px-2.5 py-1 text-[12px] transition-colors duration-150 outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70 ${sort === 'heard' ? 'bg-white/8 text-cream' : 'text-zinc-500 hover:text-cream'}`}
                            >how often</button>
                            <button
                                type="button"
                                onClick={() => setSort('cost')}
                                className={`rounded-lg px-2.5 py-1 text-[12px] transition-colors duration-150 outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70 ${sort === 'cost' ? 'bg-white/8 text-cream' : 'text-zinc-500 hover:text-cream'}`}
                            >what walking away saves you</button>
                        </div>
                    </div>

                    <div className="mt-4 divide-y divide-white/5 overflow-hidden rounded-2xl border border-white/8 bg-ink-950">
                        {sorted.map((objection) => <LandingObjection key={objection.who} {...objection} href="#" />)}
                    </div>

                    <p className="mt-3 font-mono text-[10px] text-zinc-700">counts are people who told us at the desk or in the returns note, Mar 2019 to Jul 2026</p>
                </section>

                <section className="mt-14">
                    <div className="flex items-baseline gap-3">
                        <h2 className="font-mono text-[11px] tracking-wider text-zinc-600 uppercase">Two machines that beat this one</h2>
                        <span className="h-px min-w-0 flex-1 bg-white/6"></span>
                    </div>

                    <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        {BEATEN.map((machine) => <LandingFeature key={machine.title} {...machine} tone="caveat" />)}
                    </div>
                </section>
                ...
                <section className="mt-12 rounded-2xl border border-white/8 bg-ink-900/50 p-5">
                    <div className="flex flex-wrap items-end justify-between gap-3">
                        <div className="max-w-lg">
                            <h2 className="text-[15px] font-medium tracking-tight text-cream">The 214 that came back, and what they said on the way out</h2>
                        ...
                        <span className="shrink-0 font-mono text-[10px] text-zinc-700">returns, Mar 2019 – Jul 2026</span>
                    </div>

                    <div className="mt-6 flex flex-col gap-5">
                        {RETURNS.map((row) => <LandingBar key={row.label} {...row} max={78} />)}
                    </div>
                </section>

                <section className="mt-12 grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                    <LandingQuote
                    ...
                    <div className="flex flex-col justify-center rounded-2xl border border-jade-500/25 bg-jade-500/5 p-5">
                        <p className="font-mono text-[10px] tracking-wider text-jade-400 uppercase">Still here after all that</p>
                        <h2 className="mt-2 text-lg font-semibold tracking-tight text-balance text-cream">Then you are the person it was made for.</h2>
                        ...
                        <div className="mt-4 flex flex-wrap items-center gap-3">
                            <a href="/templates/landing/screens/batch" target="_top" className="inline-flex items-center gap-2 rounded-xl bg-jade-500 px-4 py-2.5 text-[13px] font-medium text-ink-950 transition-colors duration-150 outline-none hover:bg-jade-400 focus-visible:ring-2 focus-visible:ring-jade-500/70">Take a place in batch 41</a>
                            <span className="font-mono text-[10px] text-zinc-600">46 left</span>
                        </div>
                    </div>
                </section>
            </div>
