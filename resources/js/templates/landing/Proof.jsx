...
export function LandingProof() {
    ...
    const set = SETS.find((entry) => entry.key === tab);
        ...
        <LandingShell active="The measurements" ribbon="Every figure below was measured in the workshop. The method is at the bottom of the page.">
            <div className="mx-auto max-w-5xl">
                <header className="max-w-2xl">
                    ...
                    <h1 className="mt-4 text-3xl leading-[1.15] font-semibold tracking-tight text-balance text-cream">Three sets of numbers, one of which we lose.</h1>
                ...
                <section className="mt-10">
                    <div className="flex w-fit flex-wrap items-center gap-1 rounded-xl border border-white/8 bg-ink-900/60 p-1">
                        {SETS.map((entry) => (
                            <button
                                key={entry.key}
                                type="button"
                                onClick={() => setTab(entry.key)}
                                className={`rounded-lg px-3 py-1.5 text-[13px] transition-colors duration-150 outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70 ${tab === entry.key ? 'bg-white/8 text-cream' : 'text-zinc-500 hover:text-cream'}`}
                            >{entry.tab}</button>
                        ))}
                    </div>
                    ...
                    <div className="mt-5">
                        <div className="flex flex-wrap items-end justify-between gap-3">
                            <div className="max-w-xl">
                                <h2 className="text-[15px] font-medium tracking-tight text-cream">{set.title}</h2>
                                <p className="mt-1.5 text-[12px]/5 text-zinc-500">{set.blurb}</p>
                            </div>
                            <span className="shrink-0 font-mono text-[10px] text-zinc-700">{set.unit}</span>
                        </div>

                        <div className="mt-6 flex flex-col gap-5 rounded-2xl border border-white/8 bg-ink-950 p-5">
                            {set.rows.map((row) => <LandingBar key={row.label} {...row} max={ceiling} />)}
                        </div>

                        <p className="mt-3 font-mono text-[10px] text-zinc-700">{set.foot}</p>
                    </div>
                </section>

                <section className="mt-14">
                    <div className="flex items-baseline gap-3">
                        <h2 className="font-mono text-[11px] tracking-wider text-zinc-600 uppercase">The drop test, including the two it failed</h2>
                        <span className="h-px min-w-0 flex-1 bg-white/6"></span>
                        <span className="shrink-0 font-mono text-[10px] text-zinc-700">five drops, 2024</span>
                    </div>

                    <div className="mt-4 overflow-hidden rounded-2xl border border-white/8 bg-ink-950">
                        {DROPS.map((drop, index) => (
                            <div
                                key={`${drop.when}-${drop.what}`}
                                className={`flex flex-col gap-2 px-4 py-3.5 sm:flex-row sm:items-baseline sm:gap-5 ${index > 0 ? 'border-t border-white/5' : ''}`}
                            >
                                <span className="shrink-0 font-mono text-[10px] text-zinc-700 sm:w-20">{drop.when}</span>

                                <div className="min-w-0 flex-1">
                                    <p className="text-[13px]/5 text-cream">{drop.what}</p>
                                    <p className="mt-1 text-[12px]/5 text-zinc-500">{drop.detail}</p>
                                </div>

                                <span className={`inline-flex shrink-0 items-center gap-1.5 rounded-lg border px-2 py-0.5 font-mono text-[10px] uppercase ${OUTCOMES[drop.tone]}`}>{drop.outcome}</span>
                            </div>
                        ))}
                    </div>
                </section>

                <section className="mt-12 grid grid-cols-1 gap-3 lg:grid-cols-3">
                    {METHOD.map((card) => <LandingFeature key={card.title} {...card} />)}
                </section>

                <section className="mt-12 rounded-2xl border border-white/8 bg-ink-900/50 p-5">
                    <h2 className="text-[15px] font-medium tracking-tight text-cream">The part of this page that argues against us</h2>
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3">
                        ...
                        <span className="font-mono text-[10px] text-zinc-700">we name it, and where to get it</span>
                    </div>
                </section>
            </div>
        </LandingShell>
