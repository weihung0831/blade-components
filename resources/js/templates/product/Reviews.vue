...
import UiRating from '../../components/ui/forms/Rating.vue';
import ProductShell from './Shell.vue';
import ProductReviewBar from './ReviewBar.vue';
...
const visible = computed(() => reviews.filter((review) => {
    if (filter.value === 'all') {
        return true;
    }

    if (filter.value === 'photos' || filter.value === 'verified') {
        return review[filter.value];
    }

    return review.stars === Number(filter.value);
}));
</script>

<template>
    <ProductShell v-model:finish="finish" active="Reviews">
            ...
            <div class="flex flex-col gap-4 lg:sticky lg:top-32">
                <div class="rounded-2xl border border-white/8 bg-ink-900 p-6">
                    <div class="flex items-end gap-4">
                        ...
                        <div class="pb-1">
                            <UiRating :model-value="5" readonly class="[&>span:last-child]:hidden" />
                ...
                <div class="rounded-2xl border border-white/8 bg-ink-900 p-5">
                    ...
                    <UiButton variant="secondary" size="sm" class="mt-4 w-full">Write a review</UiButton>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-4">
                <div class="flex flex-wrap items-center gap-2 rounded-xl border border-white/8 bg-ink-900 px-4 py-3">
                    <button
                        ...
                        class="rounded-full border px-3 py-1.5 text-[12px] transition-colors duration-150 outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70"
                        :class="filter === item.key ? 'border-jade-500/50 bg-jade-500/10 text-jade-300' : 'border-white/8 text-zinc-500 hover:border-white/25 hover:text-cream'"
                    >
                        {{ item.label }}
                    </button>

                    <span class="ml-auto font-mono text-[10px] text-zinc-600">showing {{ visible.length }} of {{ reviews.length }} loaded</span>
                </div>

                <div class="flex flex-col gap-4">
                    <article v-for="review in visible" :key="review.name" class="rounded-2xl border border-white/8 bg-ink-900 p-5">
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span v-if="review.verified" class="inline-flex items-center gap-1 font-mono text-[10px] text-jade-400">
                            ...
                            <div class="ml-auto flex shrink-0 items-center gap-3">
                                <UiRating :model-value="review.stars" readonly class="[&>span:last-child]:hidden [&_svg]:size-3.5!" />
                        ...
                        <div v-if="review.photos" class="mt-4 flex gap-2">
                            <span v-for="photo in 3" :key="photo" class="dot-grid grid size-16 place-items-center rounded-lg border border-white/8 bg-ink-950">
                        ...
                        <div v-if="review.reply" class="mt-4 rounded-xl border border-white/8 bg-ink-950 p-4">
                    ...
                    <p v-if="visible.length === 0" class="rounded-2xl border border-dashed border-white/10 px-5 py-10 text-center text-[13px] text-zinc-600">
