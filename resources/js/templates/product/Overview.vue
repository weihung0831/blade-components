...
import UiAccordion from '../../components/ui/data-display/Accordion.vue';
import UiBadge from '../../components/ui/data-display/Badge.vue';
import UiButton from '../../components/ui/actions/Button.vue';
import UiInputNumber from '../../components/ui/forms/InputNumber.vue';
...
import ProductGallery from './Gallery.vue';
...
const price = computed(() => (finish.value === 'jade' ? '$1,300' : '$1,180'));
const instalment = computed(() => (finish.value === 'jade' ? '$216.67' : '$196.67'));
...
<template>
    <ProductShell v-model:finish="finish" active="Overview">
        ...
        <div class="mt-6 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_23rem]">
            ...
            <div class="flex flex-col">
                <div class="flex flex-wrap items-center gap-2">
                    <UiBadge variant="dot" color="jade" class="font-mono text-[11px]">{{ stock[finish].badge }}</UiBadge>
                    <span class="font-mono text-[11px] text-zinc-600">SKU EG83-{{ stock[finish].sku }}</span>
                </div>
                ...
                <div class="mt-6 flex items-baseline gap-2">
                    <span class="text-3xl font-semibold tracking-tight text-cream">{{ price }}</span>
                    <span class="font-mono text-xs text-zinc-600">incl. tax</span>
                </div>
                <p class="mt-1.5 font-mono text-[11px] text-zinc-600">or 6 × {{ instalment }} at 0% · no fee, no credit check</p>
                ...
                <div class="mt-6">
                    ...
                    <ProductFinishPicker v-model="finish" detailed class="mt-2.5" />
                </div>

                <div class="mt-6 flex items-stretch gap-2.5">
                    <UiInputNumber v-model="quantity" :min="1" :max="9" class="w-28! shrink-0 [&_[data-ui-number]]:h-10!" />
                    <UiButton class="h-10 flex-1">Add to cart</UiButton>
                ...
                <p class="mt-3 text-center font-mono text-[10px] text-zinc-600">
                    <span v-if="finish === 'jade'">charged when the batch ships, not now · </span>free express over $600
                </p>
        ...
        <section class="mt-14 grid gap-4 lg:grid-cols-3">
            <article v-for="highlight in highlights" :key="highlight.title" class="flex flex-col rounded-2xl border border-white/8 bg-ink-900 p-6">
        ...
        <section class="mt-12 overflow-hidden rounded-2xl border border-white/8 bg-ink-900">
            ...
            <div class="grid gap-px bg-white/8 sm:grid-cols-3 lg:grid-cols-6">
                <div v-for="spec in quickSpecs" :key="spec.label" class="bg-ink-900 p-4">
        ...
        <section class="mt-12 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_20rem]">
            <div>
                ...
                <UiAccordion :items="faq" variant="outline" class="mt-4 bg-ink-900!" />
            </div>
            ...
            <div>
                ...
                <div class="mt-4 flex flex-col divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                    <a v-for="item in together" :key="item.name" href="/templates/product/screens/configure" target="_top"
