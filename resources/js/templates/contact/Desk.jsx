...
export function ContactDesk() {
    return (
        <ContactShell
            active="The desk"
            toolbar={<ContactHours zone="Taipei" time="04:12" cursor={4.2} state="shut" windows={[[9.5, 18.5]]} note="Mail is read 09:30–18:30, Mon–Fri" />}
        >
            <div className="mx-auto max-w-4xl">
                <h1 className="text-lg font-semibold tracking-tight text-cream">Four ways in, and a person at the end of each</h1>
                ...
                <div className="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-[1.4fr_1fr]">
                    <section>
                        <h2 className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Have these to hand</h2>
                        <p className="mt-2 max-w-lg text-[13px]/6 text-zinc-500">
                        ...
                        <div className="mt-4 divide-y divide-white/5 border-y border-white/5">
                            {READY.map((entry) => (
                                <div key={entry.what} className="flex gap-4 py-3.5">
                                    <span className="w-28 shrink-0 text-[13px] text-zinc-300">{entry.what}</span>
                                    <span className="text-[12px]/5 text-zinc-500">{entry.where}</span>
                                </div>
                            ))}
                        </div>

                        <h2 className="mt-8 font-mono text-[10px] tracking-wider text-zinc-700 uppercase">What we will not do by mail</h2>
                    ...
                    <aside>
                        <h2 className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Who reads it</h2>

                        <div className="mt-4 space-y-5">
                            {PEOPLE.map((entry) => <ContactPerson key={entry.name} {...entry} />)}
                        </div>

                        <div className="mt-6 rounded-xl border border-white/8 bg-ink-900 p-4">
                            <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">By telephone</p>
                            <p className="mt-2 font-mono text-base text-cream">+886 2 2765 4418</p>
                ...
                <section className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-3 rounded-xl border border-jade-500/25 bg-jade-500/5 p-4">
                    <div className="min-w-0 flex-1">
                        <p className="text-[13px] text-cream">Six letters in ten already have an answer on this site</p>
