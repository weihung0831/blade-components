...
import AnalyticsCohort from './Cohort.vue';
import AnalyticsToken from './Token.vue';

const event = ref('order_paid');
...
<template>
    ...
    >
        <template #actions>
            <UiSelect v-model="event" :options="['order_paid', 'session_start', 'add_to_cart']" size="sm" class="w-40" />
        ...
        <UiCard>
                ...
                <div class="flex flex-wrap items-baseline justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-zinc-500">
                            <span v-for="range in ranges" :key="range" :class="show[range]">{{ grouping[range] }}</span>
                        </p>
                    </div>
            ...
            <AnalyticsCohort v-for="range in ranges" :key="range" :columns="grids[range].columns" :rows="grids[range].rows" :class="block[range]" />
        </UiCard>
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                ...
                <template #header>
                    <div class="flex flex-wrap items-baseline justify-between gap-3">
                        ...
                        <span class="font-mono text-[11px] text-zinc-600">
                            <span v-for="range in ranges" :key="range" :class="show[range]">{{ shape[range] }}</span>
                        </span>
                    </div>
                </template>
            ...
            <UiCard>
                ...
                <ul class="flex flex-col gap-4">
                    <li v-for="plan in plans" :key="plan.label">
                        ...
                        <div class="mt-1.5 h-1.5 overflow-hidden rounded-full bg-ink-950">
                            <span class="block h-full rounded-full" :class="barFor(plan.value)" :style="{ width: `${plan.value}%` }"></span>
                        </div>
