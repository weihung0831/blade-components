...
const jump = (id) => scroller?.scrollTo({ top: offsetOf(document.getElementById(id)) - 72, behavior: 'smooth' });

onMounted(() => {
    scroller = document.querySelector('[data-blog-scroll]');
...
<template>
    <BlogShell active="Latest" :progress="progress">
        <article>
            <a href="/templates/blog/screens/latest" target="_top"
            ...
            <div class="mt-7 flex flex-wrap items-center justify-between gap-4 border-y border-white/5 py-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-0.5 rounded-lg bg-ink-900 p-0.5">
                        <label
                            v-for="option in [{ value: 's', label: 'S' }, { value: 'm', label: 'M' }, { value: 'l', label: 'L' }]"
                            :key="option.value"
                            class="grid size-7 cursor-pointer place-items-center rounded-md font-mono text-[11px] text-zinc-500 transition-colors duration-150 hover:text-cream has-[:checked]:bg-white/10 has-[:checked]:text-cream"
                        >
                            <input v-model="type" type="radio" name="type-size" :value="option.value" class="sr-only" />
                            {{ option.label }}
                            <span class="sr-only">Type size {{ option.label }}</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 grid items-start gap-10 lg:grid-cols-[13rem_minmax(0,1fr)]">
                    ...
                    <ul class="mt-3 flex flex-col gap-0.5 border-l border-white/8">
                        <li v-for="section in sections" :key="section.id">
                            <a
                                ...
                                class="-ml-px block border-l py-1.5 pl-3 text-[13px]/5 transition-colors duration-150 hover:text-zinc-300"
                                :class="current === section.id ? 'border-jade-500 text-jade-300' : 'border-transparent text-zinc-600'"
                            >{{ section.label }}</a>
                        </li>
                    </ul>
                ...
                <div class="max-w-2xl space-y-6 text-zinc-400" :class="typeSizes[type]">
                    <section id="what-came-back" class="scroll-mt-20 space-y-5">
                    ...
                    <section id="measuring" class="scroll-mt-20 space-y-5">
                    ...
                    <section id="numbers" class="scroll-mt-20 space-y-5">
                        ...
                        <div class="overflow-hidden rounded-2xl border border-white/8 bg-ink-900">
                            <table class="w-full text-left">
                                ...
                                <tbody class="divide-y divide-white/5">
                                    <tr v-for="(row, index) in rows" :key="row.model" class="text-[13px] text-zinc-400">
                                        <td class="px-4 py-3 text-zinc-200">{{ row.model }}</td>
                                        <td class="px-4 py-3 font-mono">{{ row.units }}</td>
                                        <td class="px-4 py-3 font-mono" :class="index === rows.length - 1 ? 'text-red-400' : 'text-jade-300'">{{ row.runout }}</td>
                    ...
                    <section id="tuesday" class="scroll-mt-20 space-y-5">
                    ...
                    <section id="changed" class="scroll-mt-20 space-y-5">
                    ...
                    <section id="yours" class="scroll-mt-20 space-y-5">
