...
import ChangelogPromise from './Promise.vue';
...
const picked = ref('all-promises');

const shown = computed(() => promises.filter((promise) => picked.value === 'all-promises' || promise.state === picked.value));

const tally = computed(() => (picked.value === 'all-promises'
    ? 'fourteen things, nine of them out'
    : `${spell[shown.value.length]} of the fourteen`));
</script>

<template>
    <ChangelogShell active="The record">
        <template #toolbar>
            <div class="flex flex-wrap items-center gap-x-2 gap-y-2">
                ...
                <span class="ml-auto font-mono text-[10px] text-zinc-600">{{ tally }}</span>
            </div>
        </template>
        ...
        <div class="mx-auto max-w-5xl">
            ...
            <div class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div v-for="tile in tiles" :key="tile.label" class="rounded-xl border border-white/8 bg-ink-950 p-3.5">
            ...
            <section class="mt-8">
                ...
                <p v-if="shown.length === 0" class="mt-3 rounded-xl border border-white/8 bg-ink-900 px-3.5 py-6 text-center text-[12px] text-zinc-600">
            ...
            <section class="mt-9">
                ...
                <div class="mt-2.5 flex flex-col gap-3">
                    <article v-for="incident in pulled" :key="incident.version" class="overflow-hidden rounded-xl border border-red-400/20 bg-ink-950">
