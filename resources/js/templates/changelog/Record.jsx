...
import { ChangelogPromise } from './Promise';
...
const PULLED = [
    { version: '4.1.3', date: '4 Aug 2026', lived: 'four hours', what: 'Returned stock counted twice, so anything sent back read as two back on the shelf.', found: 'A shop with three grinders in stock, reading five. They wrote in at 13:40 and we had it off by 14:20.', after: 'The nightly rebuild now runs against a copy first and refuses to write if the count moves by more than 2%.', reach: '96 shops' },
    { version: '3.9.2', date: '19 Nov 2025', lived: 'forty minutes', what: 'Invoice PDFs came out white on white for anybody whose shop was set to the light theme.', found: 'Our own bookkeeper, printing an invoice.', after: 'PDFs are rendered from a fixed palette that has nothing to do with the shop theme. It should never have been reading it.', reach: 'about 400 shops, though only 30 printed anything in the window' },
    { version: '3.4.0', date: '11 Feb 2025', lived: 'three days', what: 'The search index fell behind by a few minutes at first, then by hours. New products stopped being findable in their own shop.', found: 'Nobody reported it for two days. We found it on the third by looking at a graph for a different reason, which is the part of this that still bothers us.', after: 'The index has its own lag alarm at five minutes, and the search page says how old its answers are when the lag is over a minute.', reach: 'every shop, quietly' }
];
...
export function ChangelogRecord() {
    const [picked, setPicked] = useState('all-promises');

    const shown = PROMISES.filter((promise) => picked === 'all-promises' || promise.state === picked);

    const tally = picked === 'all-promises'
        ? 'fourteen things, nine of them out'
        : `${SPELL[shown.length]} of the fourteen`;
    ...
    const toolbar = (
        <div className="flex flex-wrap items-center gap-x-2 gap-y-2">
            ...
            <span className="ml-auto font-mono text-[10px] text-zinc-600">{tally}</span>
        </div>
    );

    return (
        <ChangelogShell active="The record" toolbar={toolbar}>
            <div className="mx-auto max-w-5xl">
                <h1 className="text-lg font-semibold tracking-tight text-cream">What we said we would do, against what went out</h1>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    {TILES.map((tile) => (
                        <div key={tile.label} className="rounded-xl border border-white/8 bg-ink-950 p-3.5">
                            <p className="font-mono text-2xl tracking-tight text-cream">{tile.figure}</p>
                            <p className="mt-0.5 font-mono text-[10px] tracking-wider text-zinc-600 uppercase">{tile.label}</p>
                            <p className="mt-1.5 text-[11px]/5 text-zinc-600">{tile.note}</p>
                        </div>
                    ))}
                </div>
                ...
                <section className="mt-8">
                    <div className="flex items-baseline justify-between gap-3">
                        <h2 className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Announced, and then</h2>
                        <span className="font-mono text-[10px] text-zinc-700">the bar is the wait</span>
                    </div>
                ...
                <section className="mt-9">
                    <div className="flex items-baseline justify-between gap-3">
                        <h2 className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Three releases that came back off</h2>
                        <span className="font-mono text-[10px] text-zinc-700">out of 41</span>
                    </div>

                    <div className="mt-2.5 flex flex-col gap-3">
                        {PULLED.map((incident) => (
                            <article key={incident.version} className="overflow-hidden rounded-xl border border-red-400/20 bg-ink-950">
                                <div className="border-b border-red-400/12 bg-red-400/4 px-3.5 py-3">
                                    <ChangelogStamp
                                ...
                                <dl className="grid grid-cols-1 divide-y divide-white/5 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
                                    <div className="px-3.5 py-3">
                                        <dt className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Who saw it</dt>
                                        <dd className="mt-1.5 text-[12px]/5 text-zinc-400">{incident.reach}</dd>
                                    </div>

                                    <div className="px-3.5 py-3">
                                        <dt className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">How we found out</dt>
                                        <dd className="mt-1.5 text-[12px]/5 text-zinc-400">{incident.found}</dd>
                                    </div>

                                    <div className="px-3.5 py-3">
                                        <dt className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">What changed after</dt>
                                        <dd className="mt-1.5 text-[12px]/5 text-zinc-400">{incident.after}</dd>
                                    </div>
                                </dl>
                            </article>
                        ))}
                ...
                <div className="mt-9 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div className="rounded-xl border border-white/8 bg-ink-900 p-4">
                        <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">How the eleven weeks is worked out</p>
                    ...
                    <div className="rounded-xl border border-white/8 bg-ink-900 p-4">
                        <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Why this page exists</p>
                        <p className="mt-2 text-[12px]/5 text-zinc-400">
