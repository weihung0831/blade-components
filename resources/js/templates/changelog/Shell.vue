...
defineProps({
    active: { type: String, default: 'The log' },
    version: { type: String, default: '4.2.1' },
    ...
    <div class="flex h-full w-full flex-col overflow-hidden bg-ink-950">
        <header class="shrink-0 border-b border-white/5 bg-ink-950">
            <div class="flex h-14 items-center gap-5 px-4 sm:px-5">
                <a href="/templates/changelog/screens/releases" target="_top" class="flex shrink-0 items-center gap-2.5">
        ...
        <div class="relative flex min-h-0 flex-1">
            <aside v-if="rail" class="hidden w-60 shrink-0 flex-col justify-between overflow-y-auto border-r border-white/5 py-4 lg:flex">
                <div>
                    <p class="px-4 font-mono text-[10px] tracking-wider text-zinc-700 uppercase">41 releases since March 2023</p>

                    <ul class="mt-3">
                        <li v-for="(month, index) in months" :key="month.label">
                            <a
                                href="/templates/changelog/screens/releases"
                                target="_top"
                                class="flex items-baseline gap-2 px-4 py-1.5 text-[13px] transition-colors duration-150"
                                :class="index === 0 ? 'text-cream' : 'text-zinc-500 hover:text-cream'"
                            >
                                ...
                                <span v-if="month.pulled > 0" class="size-1.5 shrink-0 rounded-full bg-red-400/70" title="one was pulled"></span>
                            </a>
                        </li>
                    </ul>
