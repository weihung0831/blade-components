...
const paths = computed(() => icons[props.icon] ?? icons.mail);
</script>
...
<template>
    <button
        ...
        class="group/channel flex w-full items-start gap-3 rounded-xl border border-white/8 bg-ink-950 p-3.5 text-left transition-colors duration-150 outline-none hover:border-white/15 focus-visible:ring-2 focus-visible:ring-jade-500/70 data-on:border-jade-500/40 data-on:bg-jade-500/5"
        @click="$emit('toggle')"
    >
        <span class="mt-0.5 grid size-8 shrink-0 place-items-center rounded-lg border border-white/8 text-zinc-500 transition-colors duration-150 group-data-on/channel:border-jade-500/40 group-data-on/channel:text-jade-300">
            <svg class="size-4.5" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round">
                <path v-for="path in paths" :key="path.d" :d="path.d" :fill="path.fill" />
            </svg>
        </span>
        ...
        <span class="min-w-0 flex-1">
            ...
            <span v-if="note" class="mt-1.5 block text-[11px]/5 text-zinc-500">{{ note }}</span>

            <span v-if="volume || lag" class="mt-2 flex flex-wrap items-baseline gap-x-3 font-mono text-[10px] text-zinc-700">
                <span v-if="volume">{{ volume }}</span>
                <span v-if="lag">{{ lag }}</span>
            </span>
        </span>
    </button>
</template>
