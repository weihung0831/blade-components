<script setup>
import ChangelogDiff from './Diff.vue';
...
<template>
    <ChangelogShell active="One release">
        <template #toolbar>
            <div class="flex flex-wrap items-center gap-x-3 gap-y-2">
                ...
                <div class="ml-auto flex items-center gap-1">
                    <a href="/templates/changelog/screens/release" target="_top" class="rounded-lg px-2.5 py-1 font-mono text-[11px] text-zinc-500 transition-colors duration-150 hover:bg-white/5 hover:text-cream">← 4.1.4</a>
                    <a href="/templates/changelog/screens/release" target="_top" class="rounded-lg px-2.5 py-1 font-mono text-[11px] text-zinc-500 transition-colors duration-150 hover:bg-white/5 hover:text-cream">4.2.1 →</a>
                </div>
            </div>
        </template>
            ...
            <div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[1.7fr_1fr]">
                <div>
                        ...
                        <div class="border-t border-white/5 px-3.5 py-3">
                            ...
                            <ol class="mt-3 flex flex-col gap-3">
                                <li v-for="(step, index) in steps" :key="step.label" class="flex gap-3">
                                    <span class="mt-0.5 grid size-5 shrink-0 place-items-center rounded-full border border-white/12 font-mono text-[10px] text-zinc-500">{{ index + 1 }}</span>
                    ...
                    <section class="mt-7">
                        ...
                        <div class="mt-2.5 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-950">
                            <ChangelogEntry v-for="entry in rest" :key="entry.title" v-bind="entry" />
                        </div>
                    </section>
                    ...
                    <section class="mt-7 overflow-hidden rounded-xl border border-white/8 bg-ink-950">
                        ...
                        <ul class="divide-y divide-white/5">
                            <li v-for="hop in rollout" :key="hop.when" class="flex items-baseline gap-3 px-3.5 py-2.5">
                                <span class="mt-1 size-1.5 shrink-0 rounded-full" :class="hop.state === 'done' ? 'bg-jade-500' : 'bg-amber-400'"></span>
                ...
                <aside>
                    ...
                    <div class="mt-4 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                        ...
                        <ul class="divide-y divide-white/5">
                            <li v-for="source in sources" :key="source.who" class="px-4 py-3">
