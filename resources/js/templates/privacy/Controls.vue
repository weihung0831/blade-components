...
import PrivacyConsent from './Consent.vue';
...
const on = ref({ remembering: false, counting: false, mail: false });

const isOn = (item) => item.state === 'locked' || on.value[item.key] === true;

const count = computed(() => `${switches.filter(isOn).length} of ${switches.length} on`);

const setAll = (wanted) => {
    Object.keys(on.value).forEach((key) => {
        on.value[key] = wanted;
    });
};
</script>
...
<template>
    <PrivacyShell active="Your switches">
        <template #toolbar>
            <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
                <span class="font-mono text-[10px] text-zinc-500">{{ count }}</span>
                ...
                <div class="ml-auto flex items-center gap-1.5">
                    <button
                        type="button"
                        class="rounded-lg border border-white/12 px-2.5 py-1 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-white/25 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70"
                        @click="setAll(false)"
        ...
        <div class="mx-auto max-w-4xl">
                ...
                <div class="mt-3 rounded-xl border border-white/8 bg-ink-800 p-4">
                    <div class="flex flex-col gap-3.5 sm:flex-row sm:items-center sm:gap-5">
                        ...
                        <div class="flex shrink-0 gap-2">
                            <button
                                type="button"
                                class="rounded-lg border border-white/15 px-3.5 py-1.5 text-[13px] text-zinc-200 transition-colors duration-150 outline-none hover:border-white/30 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70"
                                @click="setAll(false)"
                            >No thanks</button>
                            <button
                                type="button"
                                class="rounded-lg bg-jade-500 px-3.5 py-1.5 text-[13px] font-medium text-ink-950 transition-colors duration-150 outline-none hover:bg-jade-400 focus-visible:ring-2 focus-visible:ring-jade-500/70"
                                @click="setAll(true)"
            ...
            <section class="mt-9">
                ...
                <div class="mt-3 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                    <div v-for="row in outside" :key="row.name" class="flex flex-col gap-1.5 px-4 py-3 sm:flex-row sm:items-baseline sm:gap-5">
