export function PrivacyParcel({ reference, kind = 'A copy of everything', asked, due, steps = [], stage = 0, note = null }) {
    return (
        <article className="overflow-hidden rounded-xl border border-jade-500/25 bg-jade-500/5">
            <div className="flex flex-wrap items-baseline gap-x-5 gap-y-2 border-b border-jade-500/15 px-4 py-3">
                <div className="min-w-0 flex-1">
                    <p className="text-[13px] text-cream">{kind}</p>
                    <p className="mt-0.5 font-mono text-[10px] text-zinc-500">{reference}</p>
                </div>

                <div className="text-right">
                    <p className="font-mono text-[11px] text-jade-300">{due}</p>
                    <p className="mt-0.5 font-mono text-[10px] text-zinc-600">asked {asked}</p>
                </div>
            </div>
