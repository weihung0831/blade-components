...
import { PrivacyParcel } from './Parcel';
import { PrivacyTrail } from './Trail';
...
const TOOLBAR = (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
        <span className="font-mono text-[10px] text-zinc-500">PR-2026-0412 · gathered, waiting to be read</span>
        <span className="hidden font-mono text-[10px] text-zinc-700 sm:inline">the law allows 30 days · the last twelve took four</span>
        <a
...
export function PrivacyRequest() {
    return (
        <PrivacyShell active="Ask for it" toolbar={TOOLBAR}>
            <div className="mx-auto max-w-6xl">
                <h1 className="text-lg font-semibold tracking-tight text-cream">Ask for a copy, or ask us to forget</h1>
                ...
                <div className="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-[1.6fr_1fr]">
                    <section>
                        ...
                        <h2 className="mt-8 text-[15px] font-medium tracking-tight text-cream">What arrives</h2>
                        ...
                        <div className="mt-3 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                            {FILES.map((file) => (
                                <div key={file.name} className="flex flex-col gap-1.5 px-3.5 py-3 sm:flex-row sm:items-baseline sm:gap-5">
                                    <p className="w-full shrink-0 font-mono text-[11px] text-jade-400 sm:w-44">{file.name}</p>
                                    <p className="min-w-0 flex-1 text-[12px]/5 text-zinc-400">{file.what}</p>
                                    <p className="shrink-0 font-mono text-[10px] text-zinc-600">{file.size}</p>
                                </div>
                            ))}
                        </div>

                        <h2 className="mt-8 text-[15px] font-medium tracking-tight text-cream">What stays behind, and what pins it</h2>
                        <p className="mt-1 max-w-2xl text-[12px]/5 text-zinc-500">
                        ...
                        <div className="mt-3 divide-y divide-white/5 overflow-hidden rounded-xl border border-amber-400/20 bg-amber-400/5">
                            {STAYS.map((row) => (
                                <div key={row.name} className="flex flex-col gap-1.5 px-3.5 py-3 sm:flex-row sm:items-baseline sm:gap-5">
                                    <p className="w-full shrink-0 text-[13px] text-cream sm:w-52">{row.name}</p>
                                    <p className="min-w-0 flex-1 text-[12px]/5 text-zinc-400">{row.why}</p>
                                    <p className="shrink-0 font-mono text-[10px] text-amber-300/80">{row.until}</p>
                                </div>
                            ))}
                        </div>

                        <div className="mt-6 flex flex-col gap-3 rounded-xl border border-white/8 bg-ink-900 p-4 sm:flex-row sm:items-center sm:gap-5">
                            <div className="min-w-0 flex-1">
                                <p className="text-[13px] text-cream">Start one</p>
                            ...
                            <div className="flex shrink-0 gap-2">
                                <button type="button" className="rounded-lg border border-white/15 px-3.5 py-1.5 text-[13px] text-zinc-200 transition-colors duration-150 outline-none hover:border-white/30 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Erase it</button>
                                <button type="button" className="rounded-lg bg-jade-500 px-3.5 py-1.5 text-[13px] font-medium text-ink-950 transition-colors duration-150 outline-none hover:bg-jade-400 focus-visible:ring-2 focus-visible:ring-jade-500/70">Send me a copy</button>
                            </div>
                        </div>
                    </section>

                    <aside>
                        <h2 className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Who opened your record</h2>
                        ...
                        <ul className="mt-3 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                            {TRAIL.map((entry) => (
                                <PrivacyTrail key={entry.record + entry.when} {...entry} flagged={entry.flagged ?? false} />
                            ))}
                        </ul>
                        ...
                        <div className="mt-7 rounded-xl border border-white/8 bg-ink-900 p-4">
                            <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">If we get it wrong</p>
                        ...
                        <div className="mt-4 rounded-xl border border-white/8 bg-ink-900 p-4">
                            <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Twelve requests so far</p>
                            <p className="mt-2 text-[12px]/5 text-zinc-400">
