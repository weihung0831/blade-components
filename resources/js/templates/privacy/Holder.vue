...
const props = defineProps({
    ...
    control: { type: String, default: 'required' },
    gets: { type: Array, default: () => [] },
...
const tag = computed(() => controls[props.control] ?? controls.required);
</script>

<template>
    <article class="flex flex-col gap-3 rounded-xl border border-white/8 bg-ink-900 p-4 sm:flex-row sm:gap-5">
        ...
        <div class="min-w-0 flex-1">
            ...
            <div class="mt-1.5 flex flex-wrap gap-1">
                <span v-for="item in gets" :key="item" class="rounded border border-white/8 px-1.5 py-0.5 font-mono text-[10px] text-zinc-400">{{ item }}</span>
            </div>

            <p v-if="note" class="mt-2.5 text-[11px]/5 text-zinc-500">{{ note }}</p>
        </div>

        <div class="flex shrink-0 flex-row items-center gap-3 sm:w-40 sm:flex-col sm:items-end sm:gap-2">
            <span class="rounded border px-1.5 py-0.5 text-center font-mono text-[10px]" :class="tag.class">{{ tag.label }}</span>
            <span class="font-mono text-[10px] text-zinc-600">{{ basis }}</span>
            <span v-if="since" class="font-mono text-[10px] text-zinc-700">since {{ since }}</span>
        </div>
    </article>
</template>
