...
import { RefundVerdict } from './Verdict';
...
const TOOLBAR = (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
        <span className="font-mono text-[10px] text-zinc-500">seven years · 214 refunds · $198,400 paid back</span>
        <span className="hidden font-mono text-[10px] text-zinc-700 sm:inline">updated 12 Aug 2026</span>
        <a
...
export function RefundLedger() {
    return (
        <RefundShell active="The ledger" toolbar={TOOLBAR}>
            <div className="mx-auto max-w-6xl">
                <h1 className="text-lg font-semibold tracking-tight text-cream">Every refund we have paid, and the nine we did not</h1>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
                    {HEADLINE.map((stat) => (
                        <div key={stat.label} className="rounded-xl border border-white/8 bg-ink-900 p-3.5">
                            <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">{stat.label}</p>
                            <p className="mt-1.5 text-2xl font-semibold tracking-tight text-cream">{stat.value}</p>
                            <p className="mt-1 text-[11px]/5 text-zinc-600">{stat.meta}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[1.6fr_1fr]">
                    <section>
                        <h2 className="text-[15px] font-medium tracking-tight text-cream">What they were for</h2>
                        ...
                        <div className="mt-3 flex flex-col gap-2.5">
                            {REASONS.map((reason) => (
                                <div key={reason.label} className="flex items-center gap-4">
                                    <p className="w-52 shrink-0 truncate text-[12px] text-zinc-400">{reason.label}</p>
                                    <div className="h-2 min-w-0 flex-1 overflow-hidden rounded-full bg-white/8">
                                        <div className="h-full rounded-full bg-jade-500" style={{ width: `${reason.share}%` }}></div>
                                    </div>
                                    <p className="w-10 shrink-0 text-right font-mono text-[11px] text-zinc-500">{reason.count}</p>
                                </div>
                            ))}
                        </div>

                        <h2 className="mt-8 text-[15px] font-medium tracking-tight text-cream">Six of the nine we turned down</h2>
                        <p className="mt-1 max-w-2xl text-[12px]/5 text-zinc-500">
                    ...
                    <aside>
                        <h2 className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Three faults the returns found</h2>
                        ...
                        <div className="mt-3 flex flex-col gap-3">
                            {FIXES.map((fix) => (
                                <div key={fix.title} className="rounded-xl border border-white/8 bg-ink-900 p-3.5">
                                    <p className="text-[13px] text-cream">{fix.title}</p>
                                    <p className="mt-1.5 text-[12px]/5 text-zinc-400">{fix.body}</p>
                                    <p className="mt-2.5 font-mono text-[10px] text-zinc-700">{fix.meta}</p>
                                </div>
                            ))}
                        </div>

                        <div className="mt-7 rounded-xl border border-white/8 bg-ink-900 p-4">
                            <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Where the 23 days came from</p>
                            ...
                            <p className="mt-2.5 font-mono text-[10px] text-zinc-600">Next shutdown: 14–22 Feb 2027.</p>
                        </div>
