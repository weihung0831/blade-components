...
import RefundReason from './Reason.vue';
            ...
            <div class="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1.3fr]">
                <section>
                    ...
                    <div class="mt-6 rounded-xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <ul class="mt-2.5 space-y-2">
                            <li v-for="line in nots" :key="line" class="flex gap-2.5 text-[12px]/5 text-zinc-400">
                ...
                <section>
                    <div class="rounded-xl border border-jade-500/25 bg-jade-500/5 p-4">
                        ...
                        <div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
                            <div>
                                ...
                                <p class="mt-1 text-[13px]/5 text-cream">{{ chosen.freight }}</p>
                            </div>
                            <div>
                                <p class="font-mono text-[10px] text-zinc-600">What lands back</p>
                                <p class="mt-1 text-[13px]/5 text-cream">{{ chosen.back }}</p>
                            </div>
                            <div>
                                <p class="font-mono text-[10px] text-zinc-600">Median, start to bank</p>
                                <p class="mt-1 text-[13px]/5 text-cream">{{ chosen.days }}</p>
                            </div>
                        </div>
                        <p class="mt-3.5 border-t border-jade-500/15 pt-3 text-[12px]/5 text-zinc-400">{{ chosen.box }}</p>
                    </div>
                    ...
                    <form class="mt-5 flex flex-col gap-4">
                        ...
                        <label class="block">
                            ...
                            <textarea
                                ...
                                class="mt-1.5 w-full resize-none rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-[13px]/6 text-zinc-200 outline-none transition-colors duration-150 placeholder:text-zinc-700 focus:border-jade-500/60 focus-visible:ring-2 focus-visible:ring-jade-500/40"
                            ></textarea>
                        </label>
                    ...
                    <ol class="mt-3 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                        <li v-for="(step, index) in steps" :key="step.title" class="flex gap-3.5 px-3.5 py-3">
                            <span class="mt-0.5 shrink-0 font-mono text-[11px] text-jade-400">{{ String(index + 1).padStart(2, '0') }}</span>
