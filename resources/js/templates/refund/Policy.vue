...
import RefundRule from './Rule.vue';
import RefundWindow from './Window.vue';
...
const picked = ref('all-rules');

const shown = computed(() => rules.filter((rule) => picked.value === 'all-rules' || rule.back === picked.value));
const count = computed(() => (picked.value === 'all-rules' ? 'showing all eight' : `showing ${spell[shown.value.length]} of eight`));
</script>

<template>
    <RefundShell active="The policy">
        <template #toolbar>
            <div class="flex flex-wrap items-center gap-x-2 gap-y-2">
                ...
                <span class="ml-auto font-mono text-[10px] text-zinc-600">{{ count }}</span>
            </div>
        </template>
                ...
                <section>
                    ...
                    <div class="mt-6 rounded-xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                            <div v-for="[label, value] in deductions" :key="label" class="rounded-lg border border-white/8 px-2.5 py-2">
