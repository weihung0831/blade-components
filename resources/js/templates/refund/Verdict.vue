...
const props = defineProps({
    ...
    outcome: { type: String, default: 'refused' },
});
...
const tag = computed(() => outcomes[props.outcome] ?? outcomes.refused);
</script>

<template>
    <li class="flex gap-3 px-3.5 py-3" :class="{ 'bg-jade-500/5': outcome === 'wrong' }">
        <span class="mt-1.5 size-1.5 shrink-0 rounded-full" :class="tag.dot"></span>

        <span class="min-w-0 flex-1">
            <span class="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                <span class="text-[13px] text-zinc-300">{{ asked }}</span>
                <span class="font-mono text-[10px]" :class="tag.class">{{ tag.label }}</span>
            </span>
