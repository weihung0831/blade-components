...
import OnboardingMapping from './Mapping.vue';
...
const picked = ref('all-columns');
const duplicate = ref('skip');

const shown = computed(() => columns.filter((column) => (keeps[picked.value] ?? keeps['all-columns'])(column.state)));
</script>

<template>
    <OnboardingShell active="Bringing it over" step="catalog" interactive>
        <template #toolbar>
            ...
            <div class="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-[1.7fr_1fr]">
                <section>
                    <div class="flex items-baseline justify-between gap-3">
                        ...
                        <span class="font-mono text-[10px] text-zinc-600">{{ shown.length }} of {{ columns.length }}</span>
                    </div>
                    ...
                    <div class="mt-6 rounded-xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <div class="mt-3 flex flex-col gap-2">
                            <label
                                v-for="option in duplicates"
                                :key="option.key"
                                class="flex cursor-pointer gap-3 rounded-lg border p-3 transition-colors duration-150"
                                :class="duplicate === option.key ? 'border-jade-500/50 bg-jade-500/6' : 'border-white/8 hover:border-white/15'"
                            >
                                <input type="radio" name="duplicates" class="sr-only" :checked="duplicate === option.key" @change="duplicate = option.key" />
                                <span class="mt-0.5 size-3.5 shrink-0 rounded-full border-2" :class="duplicate === option.key ? 'border-jade-500 bg-jade-500/30' : 'border-white/20'"></span>
                ...
                <aside>
                    <div class="rounded-xl border border-jade-500/25 bg-jade-500/5 p-4">
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-3">
                            <div v-for="[label, value] in summary" :key="label" class="rounded-lg border border-white/8 bg-ink-950/40 px-2.5 py-2">
                    ...
                    <div class="mt-4 rounded-xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <ul class="mt-3 flex flex-col gap-3">
                            <li v-for="entry in refused" :key="entry.reason">
