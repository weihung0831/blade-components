...
import TermsNotice from './Notice.vue';
import TermsRevision from './Revision.vue';
import TermsDiff from './Diff.vue';
...
const picked = ref('4.2');

const shown = computed(() => versions.find((version) => version.version === picked.value));
</script>

<template>
    <TermsShell active="What changed" :rail="false">
        <template #toolbar>
            <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
                ...
                <a
                    ...
                    target="_top"
                    class="ml-auto font-mono text-[11px] text-jade-400 transition-colors duration-150 hover:text-jade-300"
                >read 4.1 in full →</a>
            </div>
        </template>
            ...
            >
                <template #actions>
                    <a
                        ...
                        target="_top"
                        class="rounded-lg border border-white/12 px-3 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream"
            ...
            <div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,18rem)_1fr]">
                ...
                <div>
                    <div class="flex flex-wrap items-baseline gap-x-3 gap-y-1.5">
                        <h2 class="text-[15px] font-medium tracking-tight text-cream">What {{ shown.version }} did</h2>
                        <span class="font-mono text-[11px] text-zinc-700">{{ shown.date }}</span>
                    </div>

                    <p class="mt-2 max-w-2xl text-[13px]/6 text-zinc-500">{{ shown.lead }}</p>
                    ...
                    <p class="mt-3 flex flex-wrap items-baseline gap-x-2 gap-y-1">
                        ...
                        <span class="text-[12px]/5 text-zinc-400">{{ shown.asked }}</span>
                    </p>

                    <p v-if="shown.note" class="mt-5 rounded-xl border border-white/8 bg-ink-900 p-4 text-[12px]/5 text-zinc-500">{{ shown.note }}</p>

                    <div v-if="shown.diffs.length" class="mt-5 flex flex-col gap-3">
                        <TermsDiff
                            v-for="diff in shown.diffs"
                            :key="diff.clause"
                            :clause="diff.clause"
                            :title="diff.title"
                            :lines="diff.lines"
                    ...
                    <section class="mt-8 rounded-xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <div class="mt-3.5 grid grid-cols-1 gap-x-6 gap-y-3.5 sm:grid-cols-2">
                            <div v-for="entry in how" :key="entry.what" class="border-l border-white/8 pl-3">
