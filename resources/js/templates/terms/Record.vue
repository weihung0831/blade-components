...
import TermsStamp from './Stamp.vue';
...
<template>
    <TermsShell active="Your copy" :rail="false">
        <template #toolbar>
            <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
                ...
                <a
                    ...
                    target="_top"
                    class="ml-auto font-mono text-[11px] text-amber-300/90 transition-colors duration-150 hover:text-amber-300"
            ...
            <div class="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-[1.5fr_1fr]">
                <section class="flex flex-col gap-6">
                    ...
                    >
                        <template #actions>
                            ...
                            <button
                                type="button"
                                class="rounded-lg border border-white/12 px-3 py-1.5 text-[12px] transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70"
                                :class="copied ? 'text-jade-400' : 'text-zinc-300'"
                                @click="copy"
                    ...
                    >
                        <template #actions>
                            <button
                                type="button"
                                class="rounded-lg bg-jade-500 px-3 py-1.5 text-[12px] font-medium text-ink-950 transition-colors duration-150 outline-none hover:bg-jade-400 focus-visible:ring-2 focus-visible:ring-jade-500/70"
                                @click="takenEarly = true"
                            >Take 4.2 now</button>
                    ...
                    <section>
                        ...
                        <div class="mt-3.5 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                            <div v-for="order in orders" :key="order.ref" class="flex flex-wrap items-baseline gap-x-4 gap-y-1 px-3.5 py-3">
                                ...
                                <span
                                    ...
                                    :class="order.state === 'current' ? 'border-jade-500/40 bg-jade-500/10 text-jade-300' : 'border-white/10 text-zinc-600'"
                ...
                <aside>
                    ...
                    <div class="mt-3 divide-y divide-white/5 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                        <div v-for="entry in history" :key="entry.version" class="flex items-baseline gap-3 px-3 py-2.5">
                            <span class="font-mono text-[12px]" :class="entry.yours ? 'text-cream' : 'text-zinc-600'">{{ entry.version }}</span>
                            <span v-if="entry.yours" class="size-1.5 shrink-0 rounded-full bg-jade-400"></span>
                    ...
                    <div class="mt-3 space-y-3.5">
                        <div v-for="entry in kept" :key="entry.what" class="border-l border-white/8 pl-3">
