...
const station = ref('Queued');
const comment = ref('');

const items = computed(() => checklist.value.flatMap((group) => group.items));
const done = computed(() => items.value.filter((item) => item.done).length);
const next = computed(() => stations[Math.min(stations.length - 1, stations.indexOf(station.value) + 1)]);

const advance = () => {
    station.value = next.value;
};
...
const post = () => {
    ...
    activity.value.push({ who: 'Mei Tsai', when: 'just now', body: comment.value.trim() });
    comment.value = '';
};
</script>
    ...
    <KanbanShell active="Board">
        <div class="mx-auto w-full max-w-5xl">
            <nav class="flex flex-wrap items-center gap-2 font-mono text-[11px] text-zinc-600">
                <a href="/templates/kanban/screens/board" target="_top" class="transition-colors duration-150 hover:text-cream">Shop floor</a>
                <span aria-hidden="true">/</span>
                <a href="/templates/kanban/screens/board" target="_top" class="transition-colors duration-150 hover:text-cream">{{ station }}</a>
                <span aria-hidden="true">/</span>
            ...
            <div class="mt-6 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_19rem]">
                <div class="flex flex-col gap-5">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div class="flex items-center gap-2.5">
                                <span class="block h-1 w-24 overflow-hidden rounded-full bg-white/10">
                                    <span class="block h-full rounded-full bg-jade-500 transition-[width] duration-300 ease-snap" :style="{ width: `${(done / items.length) * 100}%` }"></span>
                                </span>
                                <span class="font-mono text-[11px] text-zinc-500">{{ done }}/{{ items.length }}</span>
                            </div>
                        </div>

                        <div class="mt-4 flex flex-col gap-5">
                            <div v-for="group in checklist" :key="group.group">
                                <p class="font-mono text-[10px] text-zinc-600">{{ group.group }}</p>
                                <ul class="mt-2 flex flex-col">
                                    <li v-for="item in group.items" :key="item.label">
                                        <label class="flex cursor-pointer items-start gap-2.5 rounded-lg px-2 py-1.5 transition-colors duration-150 hover:bg-white/5">
                                            <input v-model="item.done" type="checkbox" class="peer sr-only">
                    ...
                    <section class="overflow-hidden rounded-2xl border border-white/8 bg-ink-900">
                        ...
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-white/5">
                                <tr v-for="row in measurements" :key="row.serial" class="font-mono text-[11px]">
                                    <td class="px-5 py-2.5 text-zinc-400">{{ row.serial }}</td>
                                    <td class="px-5 py-2.5 text-red-300">{{ row.before }}</td>
                                    <td class="px-5 py-2.5" :class="row.state === 'done' ? 'text-jade-300' : 'text-zinc-700'">{{ row.after }}</td>
                    ...
                    <section class="rounded-2xl border border-white/8 bg-ink-900 p-5">
                        ...
                        <ol class="mt-4 flex flex-col gap-4 border-l border-white/8 pl-5">
                            <li v-for="(entry, index) in activity" :key="index" class="relative">
                                <span aria-hidden="true" class="absolute top-2 -left-[1.4rem] size-1.5 rounded-full" :class="entry.when === 'just now' ? 'bg-jade-400' : 'bg-white/20'"></span>
                        ...
                        <form class="mt-5 flex flex-col gap-2.5 border-t border-white/5 pt-5" @submit.prevent="post">
                            ...
                            <textarea
                                ...
                                class="w-full resize-none rounded-xl border border-white/10 bg-ink-950 px-3.5 py-3 text-[13px]/6 text-cream placeholder:text-zinc-600 focus:border-jade-500/60 focus:outline-none"
                            ></textarea>
                            <div class="flex items-center gap-3">
                                <p class="font-mono text-[10px] text-zinc-600">Notes go on the serial card too</p>
                                <button type="submit" class="ml-auto rounded-lg bg-jade-500 px-3 py-1.5 text-[13px] font-medium text-ink-950 transition-colors duration-150 outline-none hover:bg-jade-400 focus-visible:ring-2 focus-visible:ring-jade-500/70">Post note</button>
                            </div>
                        </form>
                    </section>
                </div>
                ...
                <aside class="flex flex-col gap-4 lg:sticky lg:top-4">
                    <section class="rounded-2xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <div class="mt-2.5 flex flex-col gap-1">
                            <label
                                ...
                                :key="entry"
                                class="flex cursor-pointer items-center gap-2.5 rounded-lg px-2 py-1.5 text-[13px] transition-colors duration-150 hover:bg-white/5"
                                :class="station === entry ? 'bg-jade-500/10' : ''"
                            >
                                <input v-model="station" type="radio" name="ticket-station" :value="entry" class="sr-only">
                                <span class="size-1.5 rounded-full" :class="station === entry ? 'bg-jade-400' : 'bg-white/20'"></span>
                                <span :class="station === entry ? 'text-jade-300' : 'text-zinc-400'">{{ entry }}</span>
                                <span v-if="index === 0" class="ml-auto font-mono text-[10px] text-zinc-600">since Friday</span>
                            </label>
                        </div>
                    </section>
                    ...
                    <section class="rounded-2xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <dl class="mt-4 flex flex-col gap-2.5 border-t border-white/5 pt-4">
                            <div v-for="fact in facts" :key="fact.label" class="flex items-baseline gap-3">
                    ...
                    <section class="rounded-2xl border border-red-400/25 bg-red-500/5 p-4">
                        ...
                        <a href="/templates/kanban/screens/board" target="_top" class="mt-2.5 block">
                    ...
                    <section class="rounded-2xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <ul class="mt-3 flex flex-col divide-y divide-white/5">
                            <li v-for="part in parts" :key="part.code" class="flex items-baseline gap-2.5 py-2.5 first:pt-0 last:pb-0">
