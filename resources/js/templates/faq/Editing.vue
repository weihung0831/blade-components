...
import FaqQuery from './Query.vue';
    ...
    <FaqShell active="Editing" :rail="false">
        <div class="mx-auto max-w-5xl">
            ...
            <div class="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
                <div v-for="stat in stats" :key="stat.label" class="rounded-xl border border-white/8 bg-ink-900 px-4 py-3.5">
                    <p class="font-mono text-2xl" :class="stat.tone">{{ stat.value }}</p>
            ...
            <div class="mt-8 grid grid-cols-1 gap-6 xl:grid-cols-5">
                <section class="xl:col-span-3">
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <div class="ml-auto flex items-center gap-1">
                            <button
                                ...
                                :key="entry.key"
                                type="button"
                                class="rounded-lg px-2.5 py-1 font-mono text-[10px] transition-colors duration-150"
                                :class="filter === entry.key ? 'bg-jade-500/15 text-jade-300' : 'text-zinc-500 hover:bg-white/5 hover:text-cream'"
                    ...
                    <div class="mt-3 overflow-hidden rounded-xl border border-white/8 bg-ink-900">
                        ...
                        <div class="flex items-center gap-3 px-4 py-2.5">
                            ...
                            <a href="/templates/faq/screens/ask" target="_top" class="ml-auto font-mono text-[10px] text-zinc-600 transition-colors duration-150 hover:text-cream">write one of these →</a>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 rounded-xl border border-white/8 bg-ink-900 p-4">
                        ...
                        <div class="mt-3.5 space-y-3">
                            <div v-for="entry in fromDesk" :key="entry.q" class="flex items-start gap-3 border-t border-white/5 pt-3">
                ...
                <section class="xl:col-span-2">
                    ...
                    <div class="mt-3 space-y-3">
                        <div v-for="entry in failing" :key="entry.q" class="rounded-xl border border-white/8 bg-ink-900 p-4">
                            <p class="text-[13px]/5 text-zinc-300">{{ entry.q }}</p>

                            <div class="mt-2.5 flex items-center gap-2.5">
                                <span class="block h-1 flex-1 overflow-hidden rounded-full bg-white/8">
                                    <span class="block h-full rounded-full" :class="entry.helpful < 70 ? 'bg-red-400/60' : 'bg-amber-400/60'" :style="{ width: `${entry.helpful}%` }"></span>
                                </span>
                                <span class="shrink-0 font-mono text-[10px]" :class="entry.helpful < 70 ? 'text-red-300' : 'text-amber-300'">{{ entry.helpful }}% of {{ entry.votes }}</span>
                            </div>
                            ...
                            <div class="mt-3 flex items-center gap-2.5">
                                ...
                                <span
                                    class="ml-auto rounded-full px-2 py-0.5 font-mono text-[10px]"
                                    :class="entry.owner === 'unclaimed' ? 'bg-amber-400/10 text-amber-300' : 'bg-white/6 text-zinc-500'"
