...
export function FaqQuestions() {
    ...
    const listed = QUESTIONS.filter((entry) => {
        const haystack = `${entry.q} ${entry.topic}`.toLowerCase();

        return (topic === null || entry.topic === topic)
            && (term.trim() === '' || haystack.includes(term.trim().toLowerCase()));
    });

    const opened = (entry) => forced === true || (listed.length === 1 && term.trim() !== '' && listed[0] === entry);
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
                <div className="shrink-0 border-b border-white/5 px-5 py-5">
                    <div className="flex flex-wrap items-end justify-between gap-4">
                        <div>
                            <h1 className="text-lg font-semibold tracking-tight text-cream">Sixteen answers</h1>
                            <p className="mt-1 max-w-lg text-[13px]/6 text-zinc-500">
                        ...
                        <button
                            type="button"
                            onClick={() => setForced(forced === true ? false : true)}
                            className="shrink-0 rounded-lg border border-white/10 px-3 py-1.5 font-mono text-[11px] text-zinc-500 transition-colors duration-150 hover:border-jade-500/50 hover:text-cream"
                        >
                            {forced === true ? 'close every one' : 'open every one'}
                        </button>
                    </div>

                    <label className="relative mt-4 flex max-w-xl items-center">
                        ...
                        <input
                            ...
                            className="w-full rounded-xl border border-white/10 bg-ink-900 py-2.5 pr-3 pl-10 text-[13px] text-cream placeholder:text-zinc-600 focus:border-jade-500/60 focus:outline-none"
                        />
                        <span className="sr-only">Search the answers</span>
                    </label>

                    <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                        <span className="font-mono text-[10px] text-zinc-700">this week</span>
                        {SEARCHED.map((word) => (
                            <button
                                key={word}
                                type="button"
                                onClick={() => setTerm(term.trim().toLowerCase() === word ? '' : word)}
                ...
                <div className="min-h-0 flex-1 overflow-y-auto">
                    <div className="grid grid-cols-1 gap-2.5 px-5 py-5 sm:grid-cols-2 xl:grid-cols-3">
                        {TOPICS.map((entry) => (
                            <FaqTopic
                                key={entry.name}
                                name={entry.name}
                                ...
                                onSelect={() => setTopic(topic === entry.name ? null : entry.name)}
                            />
                        ))}
                    </div>

                    <div className="flex items-center gap-3 border-y border-white/5 bg-ink-900/40 px-5 py-2">
                        <p className="font-mono text-[10px] text-zinc-600">
                            showing <span className="text-zinc-400">{listed.length}</span> of {QUESTIONS.length}
                            {topic && <span className="text-zinc-700"> · {topic}</span>}
                        </p>
                        {(topic !== null || term.trim() !== '') && (
                            <button
                                type="button"
                                onClick={() => { setTerm(''); setTopic(null); }}
                                className="ml-auto font-mono text-[10px] text-zinc-600 transition-colors duration-150 hover:text-cream"
                            >
                                clear
                            </button>
                        )}
                    ...
                    <div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-3">
                                        {entry.full && (
                                            <a
                                                ...
                                                target="_top"
                                                className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 px-3 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream"
                                            >
                                                The whole thing, with photographs
                                                <svg className="size-3" viewBox="0 0 16 16" fill="none"><path d="M6 3.5 10.5 8 6 12.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                                            </a>
                                        )}

                                        <span className="font-mono text-[10px] text-zinc-700">opened {entry.opens.toLocaleString()} times</span>

                                        <a href="/templates/faq/screens/ask" target="_top" className="ml-auto font-mono text-[10px] text-zinc-600 transition-colors duration-150 hover:text-cream">this did not answer it →</a>
                                    </div>
                                }
                            >
                                {entry.a.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}

                                {entry.note && (
                                    <FaqCallout tone={entry.note.tone} label={entry.note.label} className="mt-4">
                                        <p>{entry.note.text}</p>
                                    </FaqCallout>
                                )}
                            </FaqQuestion>
                        ...
                        {listed.length === 0 && (
                            <div className="px-5 py-16 text-center">
                                <p className="font-mono text-[11px] text-zinc-600">Nobody has written that one down yet.</p>
                                <p className="mx-auto mt-2 max-w-sm text-[12px]/5 text-zinc-700">
                                    Which is worth knowing on its own — every search that lands here goes on a list, and the list is what gets written next.
                                </p>
                                <a href="/templates/faq/screens/ask" target="_top" className="mt-4 inline-block rounded-lg bg-jade-500 px-3.5 py-2 text-[13px] font-medium text-ink-950 transition-colors duration-150 hover:bg-jade-400">Ask the desk instead</a>
                            </div>
                        )}
                    </div>

                    <div className="px-5 py-8">
                        <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-white/8 bg-ink-900 px-5 py-4">
                            <div>
                                <p className="text-[13px] text-zinc-300">Still not it?</p>
                                <p className="mt-1 text-[12px]/5 text-zinc-600">Nine questions found nothing last month. Yours might be the tenth, and we would rather hear it.</p>
                            </div>
                            <a href="/templates/faq/screens/ask" target="_top" className="shrink-0 rounded-lg border border-white/10 px-3.5 py-2 text-[13px] text-zinc-300 transition-colors duration-150 hover:border-jade-500/60 hover:text-cream">Write to the desk</a>
                        </div>
                    </div>
                </div>
            </div>
