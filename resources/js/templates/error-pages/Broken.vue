...
import ErrorPagesReference from './Reference.vue';
...
const notifyLabel = computed(() => (notify.value ? 'We will mail you' : 'Mail me when it is finished'));
const notifyNote = computed(() => (notify.value
    ? 'goes to the address on NS-24817, once, and that is the end of it'
    : 'one mail, to the address on the order'));
</script>

<template>
    <ErrorPagesShell active="Our fault" state="down" reference="req_2c81f0d3 · 500 · 04:19 GMT+8">
        <template #toolbar>
        ...
        <div class="mx-auto max-w-3xl">
            ...
            <section class="mt-8 overflow-hidden rounded-xl border border-jade-500/25 bg-jade-500/4">
                ...
                <div class="flex flex-wrap items-center gap-2 border-t border-jade-500/15 px-4 py-3">
                    <a href="#" target="_top" class="rounded-lg border border-jade-500/40 bg-jade-500/10 px-2.5 py-1.5 text-[12px] text-cream transition-colors duration-150 hover:border-jade-500/70">Watch NS-24817</a>

                    <button
                        type="button"
                        @click="notify = !notify"
                        class="rounded-lg border px-2.5 py-1.5 text-[12px] transition-colors duration-150 outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70"
                        :class="notify ? 'border-jade-500/50 text-cream' : 'border-white/10 text-zinc-300 hover:border-jade-500/60 hover:text-cream'"
                    >{{ notifyLabel }}</button>

                    <span class="font-mono text-[10px] text-zinc-600">{{ notifyNote }}</span>
                </div>
            </section>
                ...
                <section>
                    ...
                    <div class="mt-3 rounded-xl border border-white/8 bg-ink-950 p-3.5">
                        <ol class="flex flex-col gap-4">
                            <li v-for="(update, index) in updates" :key="update.time" class="flex gap-3">
                                <span class="flex flex-col items-center pt-1">
                                    <span class="size-1.5 shrink-0 rounded-full" :class="update.state === 'now' ? 'bg-red-400' : 'bg-white/25'"></span>
                                    <span v-if="index < updates.length - 1" class="mt-1 w-px flex-1 bg-white/8"></span>
                                </span>
