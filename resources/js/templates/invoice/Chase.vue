...
const toggle = (index) => {
    open.value = open.value.includes(index) ? open.value.filter((entry) => entry !== index) : [...open.value, index];
};
</script>

<template>
    <InvoiceShell active="Getting paid">
        <template #toolbar>
            <div class="mx-auto flex max-w-4xl flex-wrap items-center gap-3">
                ...
                <span class="ml-auto flex items-center gap-2">
                    <button type="button" class="rounded-lg border border-white/10 px-2.5 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Open the invoice</button>
                    <button type="button" class="rounded-lg border border-white/10 px-2.5 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Record a payment</button>
                </span>
            </div>
        </template>
        ...
        <div class="mx-auto max-w-4xl">
            <header class="flex flex-col gap-6 rounded-2xl border border-red-400/20 bg-red-400/4 p-6 sm:flex-row sm:items-start sm:justify-between">
                ...
                <InvoiceStamp label="Overdue" tone="overdue" note="74 days" class="shrink-0" />
            </header>
            ...
            <div class="mt-8 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
                <section>
                    ...
                    <ol class="mt-4 flex flex-col">
                        <li v-for="(event, index) in events" :key="event.title" class="flex gap-4">
                            <div class="flex shrink-0 flex-col items-center">
                                <span class="mt-1.5 size-2.5 rounded-full border" :class="dots[event.tone]"></span>
                                <span v-if="index < events.length - 1" class="w-px flex-1 bg-white/8"></span>
                            </div>

                            <div class="min-w-0 flex-1" :class="index < events.length - 1 ? 'pb-6' : ''">
                                ...
                                <div v-if="event.letter" class="mt-2.5">
                                    <button
                                        type="button"
                                        ...
                                        @click="toggle(index)"
                                    >
                                        <svg class="size-3 transition-transform duration-150" :class="open.includes(index) ? 'rotate-90' : ''" viewBox="0 0 12 12" fill="none"><path d="M4 2.5 8 6l-4 3.5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                        {{ open.includes(index) ? 'close it' : 'read what it said' }}
                                    </button>

                                    <p v-if="open.includes(index)" class="mt-2 max-w-xl rounded-xl border border-white/8 bg-ink-900 p-3.5 text-[12px]/6 text-zinc-400 italic">{{ event.letter }}</p>
                                </div>
                            </div>
                        </li>
                    </ol>
                ...
                <aside class="flex flex-col gap-3">
                    ...
                    <div class="rounded-2xl border border-white/8 bg-ink-900/50 p-5">
                        ...
                        <ul class="mt-3 flex flex-col gap-2.5">
                            <li v-for="rule in rules" :key="rule.when" class="flex gap-3">
                    ...
                    <div class="rounded-2xl border border-white/8 bg-ink-950 p-5">
                        ...
                        <dl class="mt-3 flex flex-col gap-2">
                            <div v-for="row in history" :key="row.label" class="flex items-baseline justify-between gap-4">
