...
import { InvoiceAging } from './Aging';
...
const AGES = {
    ...
    quiet: 'border-white/10 text-zinc-500',
        ...
        <InvoiceShell
            ...
            toolbar={
                <div className="mx-auto flex max-w-5xl flex-wrap items-center gap-3">
                    <span className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Receivables</span>
                    <span className="font-mono text-[11px] text-zinc-500">{shown.length} of 42 invoices shown</span>

                    <span className="ml-auto flex items-center gap-2">
                        <button type="button" className="rounded-lg border border-white/10 px-2.5 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Export the lot as CSV</button>
                        <span className="font-mono text-[10px] text-zinc-700">as at 19 Aug 2026, 08:00</span>
                    </span>
                </div>
            }
        >
            <div className="mx-auto max-w-5xl">
                <header className="flex flex-wrap items-end justify-between gap-4">
                    <div className="max-w-xl">
                        <h1 className="text-2xl font-semibold tracking-tight text-balance text-cream">NT$1,943,600 is out there, and a fifth of it is older than sixty days.</h1>
                    ...
                    <div className="text-right">
                        <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">Average days to pay</p>
                        <p className="mt-1 font-mono text-2xl tabular-nums text-cream">34</p>
                        <p className="mt-0.5 font-mono text-[10px] text-zinc-600">terms say 30 · was 41 in 2024</p>
                    </div>
                </header>

                <section className="mt-8 grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-5">
                    {BUCKETS.map((entry) => (
                        <button
                            key={entry.key}
                            type="button"
                            onClick={() => setBucket(entry.key)}
                            className="text-left outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70"
                        >
                            <InvoiceAging
                                label={entry.label}
                                count={entry.count}
                ...
                <section className="mt-8">
                    <div className="flex items-baseline gap-3">
                        <h2 className="font-mono text-[11px] tracking-wider text-zinc-600 uppercase">The eleven worth looking at</h2>
                        <span className="h-px min-w-0 flex-1 bg-white/6"></span>
                        <span className="shrink-0 font-mono text-[10px] text-zinc-700">the other 31 are under NT$20,000 each</span>
                    </div>
                    ...
                    {shown.length > 0 ? (
                            ...
                            <table className="w-full min-w-3xl border-collapse text-left">
                                <thead>
                                    <tr className="border-b border-white/8">
                                        {['invoice', 'who', 'issued', 'due', 'amount', 'late by'].map((head) => (
                                ...
                                <tbody>
                                    {shown.map((row, index) => (
                                        <tr key={row.number} className={`align-top ${index > 0 ? 'border-t border-white/5' : ''}`}>
                                            <td className="px-4 py-3 font-mono text-[12px] whitespace-nowrap text-cream">{row.number}</td>

                                            <td className="px-4 py-3">
                                                <span className="block text-[13px]/5 text-zinc-200">{row.customer}</span>
                                                <span className="mt-1 block max-w-md text-[11px]/5 text-zinc-600">{row.state}</span>
                                            </td>

                                            <td className="px-4 py-3 font-mono text-[11px] whitespace-nowrap text-zinc-500">{row.issued}</td>
                                            <td className="px-4 py-3 font-mono text-[11px] whitespace-nowrap text-zinc-400">{row.due}</td>
                                            <td className="px-4 py-3 text-right font-mono text-[12px] tabular-nums whitespace-nowrap text-zinc-200">{row.amount}</td>

                                            <td className="px-4 py-3">
                                                <span className={`inline-flex items-center rounded-lg border px-2 py-0.5 font-mono text-[10px] whitespace-nowrap ${AGES[row.tone]}`}>{row.age}</span>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                        ...
                        <p className="mt-4 rounded-2xl border border-white/8 bg-ink-900 px-4 py-8 text-center text-[12px] text-zinc-600">
                            Nothing in that bucket is over NT$20,000, so none of it is on this list.
                        </p>
                    )}
                </section>

                <section className="mt-10 grid grid-cols-1 gap-3 lg:grid-cols-3">
                    <div className="rounded-2xl border border-jade-500/25 bg-jade-500/5 p-5">
                        <p className="font-mono text-[10px] tracking-wider text-jade-400 uppercase">The big one is not the problem</p>
                    ...
                    <div className="rounded-2xl border border-white/8 bg-ink-900/50 p-5">
                        <p className="font-mono text-[10px] tracking-wider text-zinc-700 uppercase">What we stopped doing</p>
                    ...
                    <div className="rounded-2xl border border-red-400/20 bg-red-400/4 p-5">
                        <p className="font-mono text-[10px] tracking-wider text-red-400 uppercase">Written off this year</p>
                        <p className="mt-2 text-[12px]/5 text-zinc-400">
