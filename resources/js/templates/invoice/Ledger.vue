...
import InvoiceAging from './Aging.vue';
...
<template>
    <InvoiceShell active="What is owed">
        <template #toolbar>
            <div class="mx-auto flex max-w-5xl flex-wrap items-center gap-3">
                ...
                <span class="font-mono text-[11px] text-zinc-500">{{ shown.length }} of 42 invoices shown</span>

                <span class="ml-auto flex items-center gap-2">
                    <button type="button" class="rounded-lg border border-white/10 px-2.5 py-1.5 text-[12px] text-zinc-300 transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70">Export the lot as CSV</button>
        ...
        <div class="mx-auto max-w-5xl">
            ...
            <section class="mt-8 grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-5">
                <button
                    ...
                    :key="entry.key"
                    type="button"
                    class="text-left outline-none focus-visible:ring-2 focus-visible:ring-jade-500/70"
                    @click="bucket = entry.key"
                >
                    <InvoiceAging
                        :label="entry.label"
                        :count="entry.count"
            ...
            <section class="mt-8">
                ...
                <div v-if="shown.length" class="mt-4 overflow-x-auto rounded-2xl border border-white/8 bg-ink-950">
                    <table class="w-full min-w-3xl border-collapse text-left">
                        <thead>
                            <tr class="border-b border-white/8">
                                <th v-for="head in ['invoice', 'who', 'issued', 'due', 'amount', 'late by']" :key="head" class="px-4 py-2.5 font-mono text-[10px] font-normal tracking-wider text-zinc-700 uppercase">{{ head }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(row, index) in shown" :key="row.number" class="align-top" :class="index > 0 ? 'border-t border-white/5' : ''">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center rounded-lg border px-2 py-0.5 font-mono text-[10px] whitespace-nowrap" :class="ages[row.tone]">{{ row.age }}</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <p v-else class="mt-4 rounded-2xl border border-white/8 bg-ink-900 px-4 py-8 text-center text-[12px] text-zinc-600">
