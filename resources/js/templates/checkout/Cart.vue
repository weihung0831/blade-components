...
const live = computed(() => items.value.filter((item) => item.qty > 0));
const empty = computed(() => live.value.length === 0);

const inCart = (sku) => items.value.some((item) => item.sku === sku && item.qty > 0);

const setQty = (sku, qty) => {
    const item = items.value.find((candidate) => candidate.sku === sku);

    item.qty = Math.max(0, Math.min(item.max, qty));
};

const step = (item, delta) => setQty(item.sku, item.qty + delta);
</script>

<template>
    <CheckoutShell active="Cart" v-model:ship="ship">
        <div class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <div class="flex flex-col gap-6">
                ...
                <section class="overflow-hidden rounded-2xl border border-white/8 bg-ink-900">
                    ...
                    <div v-if="!empty" class="flex flex-col divide-y divide-white/5 px-5">
                        <CheckoutLineItem
                            v-for="item in live"
                            :key="item.sku"
                            :item="item"
                ...
                <section class="rounded-2xl border border-white/8 bg-ink-900 p-5">
                    ...
                    <div class="mt-4 grid gap-3 sm:grid-cols-2">
                        <div v-for="suggestion in suggestions" :key="suggestion.sku" class="flex items-start gap-3 rounded-xl border border-white/8 bg-ink-950 p-4">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="mt-2.5 flex items-center gap-3">
                                    <span class="font-mono text-[13px] text-cream">${{ suggestion.price.toLocaleString('en-US') }}</span>
                                    <button
                                        type="button"
                                        :disabled="inCart(suggestion.sku)"
                                        @click="setQty(suggestion.sku, 1)"
                                        class="ml-auto rounded-lg border border-white/10 px-2.5 py-1 font-mono text-[11px] text-zinc-400 transition-colors duration-150 outline-none hover:border-jade-500/60 hover:text-cream focus-visible:ring-2 focus-visible:ring-jade-500/70 disabled:opacity-40"
                                    >
