...
import CheckoutCardField from './CardField.vue';

const ship = ref('standard');
const pay = ref('card');
const invoice = ref('mobile');
const billingSame = ref(true);
const card = ref('4571 2000 1234 3092');
...
<template>
    <CheckoutShell active="Payment" v-model:ship="ship">
        <div class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <div class="flex flex-col gap-6">
                ...
                <section class="overflow-hidden rounded-2xl border border-white/8 bg-ink-900">
                    ...
                    <div class="grid gap-3 p-5 sm:grid-cols-2">
                        <label
                            ...
                            :key="method.value"
                            class="flex cursor-pointer items-start gap-3 rounded-xl border border-white/10 bg-ink-950 p-4 transition-colors duration-200 ease-snap hover:border-white/25 has-[:checked]:border-jade-500/50 has-[:checked]:bg-jade-500/5"
                        >
                    ...
                    <div v-if="pay === 'card' || pay === 'instalment'" class="border-t border-white/5 bg-ink-950 px-5 py-5">
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                <CheckoutCardField v-model="card" hint="Test card. Nothing here reaches a real processor." />
                            </div>

                            <UiInput label="Expiry" value="09 / 28" autocomplete="cc-exp" class="font-mono" />
                            <UiInput label="Security code" value="•••" autocomplete="cc-csc" hint="Three digits on the back, four on the front for Amex." class="font-mono" />

                            <div class="sm:col-span-2">
                                <UiInput label="Name on the card" value="WEI HAN CHEN" autocomplete="cc-name" />
                            </div>
                        </div>

                        <div v-if="pay === 'instalment'" class="mt-4 rounded-xl border border-jade-500/25 bg-jade-500/6 p-4">
                    ...
                    <div v-if="pay === 'transfer'" class="border-t border-white/5 bg-ink-950 px-5 py-5">
                    ...
                    <div v-if="pay === 'onsite'" class="border-t border-white/5 bg-ink-950 px-5 py-5">
                        <p v-if="ship !== 'pickup'" class="text-[13px]/6 text-zinc-400">
                            This one only works with workshop collection.
                            <a href="/templates/checkout/screens/delivery" target="_top" class="text-jade-400 transition-colors duration-150 hover:text-jade-300">Switch the delivery method →</a>
                        </p>
                        <p v-else class="text-[13px]/6 text-zinc-400">
                ...
                <section class="rounded-2xl border border-white/8 bg-ink-900 p-5">
                    ...
                    <div v-if="!billingSame" class="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <section class="overflow-hidden rounded-2xl border border-white/8 bg-ink-900">
                    ...
                    <div v-if="invoice === 'mobile'" class="border-t border-white/5 bg-ink-950 px-5 py-5">
                        <div class="grid gap-4 sm:grid-cols-2">
                            <UiInput label="Carrier code" value="/ABC+123" class="font-mono" hint="Eight characters, starts with a slash." />
                    ...
                    <div v-else-if="invoice === 'company'" class="border-t border-white/5 bg-ink-950 px-5 py-5">
                        <div class="grid gap-4 sm:grid-cols-2">
                            <UiInput label="Tax ID number" placeholder="24681357" inputmode="numeric" class="font-mono" />
                    ...
                    <div v-else-if="invoice === 'donate'" class="border-t border-white/5 bg-ink-950 px-5 py-5">
                        <div class="grid gap-4 sm:grid-cols-2">
                            <UiInput label="Donation code" value="25885" class="font-mono" hint="Default is the Genesis Foundation. Any registered code works." />
                            <div class="flex items-end">
                    ...
                    <div v-else class="border-t border-white/5 bg-ink-950 px-5 py-5">
            ...
            <div class="flex flex-col gap-4 lg:sticky lg:top-32">
                ...
                <div class="rounded-2xl border border-white/8 bg-ink-900 p-5">
                    ...
                    <a href="/templates/checkout/screens/delivery" target="_top"
