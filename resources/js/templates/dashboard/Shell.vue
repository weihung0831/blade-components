...
const sections = computed(() => nav.map((section) => ({
    label: section.label,
    items: section.items.map((item) => ({ ...item, active: item.label === props.active })),
})));
</script>

<template>
    <div class="flex h-full min-h-[42rem] w-full min-w-[64rem] gap-4 overflow-hidden bg-ink-950 p-4">
        <UiSidebar :sections="sections" class="shrink-0">
            <template #brand>
